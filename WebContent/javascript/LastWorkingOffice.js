...
function callServer(command) {
	...
	if (command == "search") {
		...
		var keyword = document.forms[0].searchKeyword.value;
		
		
		url = "../searchOffice.html?officeSearchObj.searchOption=" + option
				+ "&officeSearchObj.searchKeyword=" + keyword;
...
function getRow1(baseResponse) {
	...
	if (flag == "success") {
		...
		var display = baseResponse.getElementsByTagName("officename");

		for ( var i = 0; i < display.length; i++) {
			record1[i] = baseResponse.getElementsByTagName("officename")[i].firstChild.nodeValue;
			record2[i] = baseResponse.getElementsByTagName("statusdesc")[i].firstChild.nodeValue;
			record3[i] = baseResponse.getElementsByTagName("levelname")[i].firstChild.nodeValue;
			record4[i] = baseResponse.getElementsByTagName("officeid")[i].firstChild.nodeValue;
			//record5[i] = baseResponse.getElementsByTagName("levelid")[i].firstChild.nodeValue;
...
function loadRecordVal(page) {
	...
	for (i = p; i < record1.length && c < __pagination; i++) {
		...
		if (window.navigator.appName.toLowerCase().indexOf("netscape") == -1) {
			radioInput = document
					.createElement("<input type=radio name=radios value=" + sno
							+ " onclick=javascript:set(" + sno + ")>");
