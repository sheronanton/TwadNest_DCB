...
function ValChBnk(command)
{
	...
	function ValChBnk1(command)
	{
	    ...
	    if(command=="Search")
          ...
          var url="../valbanksearch.html?valbksrch.BankSearchKeyword="+BankSearchKeyword+"&valbksrch.bankSearchOptions="+bankSearchOptions;
		  alert("url is "+url);
          var req=getTransport1();
           req.open("GET",url,true);        
           req.onreadystatechange=function()
           {
               processResponseValBnkSrch(req);
            }   
            req.send(null);
            }  
 	}
		
	function processResponseValBnkSrch(req)
	{   
      if(req.readyState==4)
	  {
	      if(req.status==200)
