...
function report_open()
{
	...
	window.open('ame_bud_est_exp_cir_uprptxls.jsp'+params,'Abstract ','width=400,height=200')
} 
</script> 
</head>
...
{   
	...
	String prv_msg="TWAD HO-->"+Office_Name.trim();
	...
	while(rs.next())
	{
		...
		headhtml.append("<th>"+obj.isNull(rs.getString(2),1)+"<br>(Rs.lakhs)"+date_Display2+"<br>("+chrs[col_count-1]+")</th>");
	...
	<table width="95%" align="center"> 
		...
		<tr> 
			...
			<tr> 
				<td><%=row%></td> 
				<td><a href="ame_bud_est_exp_off_uprpt.jsp?upto_month=<%=upto_month%>&pyear=<%=inp_year%>&region_id=<%=region_id%>&Office_id=<%=Office_id%>"><%=office_name%></a></td>				
				...
				{ 
					...
					{
						amt_res_up=item_obj.getAmt_net_regprv(Integer.parseInt(Office_id),4,year,prv_month,next_year,2);
					}
					double amt_res_cur=item_obj.getAmt_net_regprv(Integer.parseInt(Office_id),Integer.parseInt(upto_month),next_year,Integer.parseInt(upto_month),next_year,2);
	...
	<tr><th colspan="2"  align="right">Total</th>
	 ...
	 <tr>
			...
			<input type="hidden" name="Office_id" value="<%=region_id%>"/>  
	<%	
		}catch(Exception e)
		{
			out.println(e); 
