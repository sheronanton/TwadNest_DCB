...
{
	var pyear,Office_id,upto_month,region_id;
	try { Office_id=document.pms_ame_reports.Office_id.value;} catch(e) {};
	try { region_id=document.pms_ame_reports.region_id.value;} catch(e) {};
	...
	params = "?pyear=" +pyear +"&Office_id=" +Office_id+"&region_id="+region_id+"&upto_month="+upto_month;
...
{  
	...
	int circle_id=Integer.parseInt(Office_id);
	br.append(" SELECT sch_.office_id,sch_.office_name as office_name,am.am_est_amt AS am_est_amt,BUDGET_EST_AMT");
	br.append(" FROM  (  select a.office_id,a.office_name from com_mst_all_offices_view  a where a.office_level_id='DN' and circle_office_id="+Office_id); 
	br.append(" 	and exists (SELECT DISTINCT office_id FROM pms_dcb_mst_beneficiary_metre WHERE meter_status='L'");
	br.append(" 	AND meter_status  ='L' and a.office_id=office_id) order by office_id )sch_");
	br.append(" 	LEFT OUTER JOIN (SELECT FIN_YEAR, OFFICE_ID, SUM(am_est_amt) AS am_est_amt FROM pms_ame_trn_abstract");
	br.append(" 	WHERE FIN_YEAR='"+fin_year+"' GROUP BY FIN_YEAR, OFFICE_ID )am");
	br.append(" 	oN am.OFFICE_ID =sch_.OFFICE_ID LEFT OUTER JOIN");
	br.append(" 	(SELECT FIN_YEAR, OFFICE_ID,SUM(BUDGET_EST_AMT) AS BUDGET_EST_AMT FROM PMS_AME_TRN_BUDGET");
	br.append(" 	WHERE FIN_YEAR='"+fin_year+"' GROUP BY FIN_YEAR, OFFICE_ID )budget ON budget.OFFICE_ID=sch_.OFFICE_ID");
	br.append(" 	aND budget.FIN_YEAR=am.FIN_YEAR  order by office_id  "); 
	...
	while(rs.next())
	{
		...
		headhtml.append("<th>"+obj.isNull(rs.getString(2),1)+"<br> Upto "+date_Display1+" (Rs.lakhs)<br>("+CHARS[col_count-1]+") </th>");
		headhtml.append("<th>"+obj.isNull(rs.getString(2),1)+"<br>"+date_Display2+"(Rs.lakhs)<br>("+chrs[col_count-1]+")</th>");
		
		if(col_count==1)
		sum_label+=CHARS[col_count-1];
		else
		sum_label+="+"+CHARS[col_count-1]; 
		 
		if(col_count==1)  
			sum_label2+=chrs[col_count-1];
			else
				sum_label2+="+"+chrs[col_count-1];
	...
	String prv_msg="TWAD HO-->"+obj.getValue("com_mst_all_offices_view", "OFFICE_NAME","where OFFICE_ID="+region_id+ "  ")+"-->"+Office_Name.trim(); 
	%>
	<table width="95%" align="center"> 
		...
		<tr> 
			...
			%>
			<th>  Total (Rs.lakhs)<br><%=sum_label%></th><th>  Total (Rs.lakhs)<br><%=sum_label2%></th></tr>
			<%			
			...
			while(rs_amt.next())
			{
				...
				Office_id=obj1.isNull(rs_amt.getString("office_id"),2);
			...
			<tr> 
				<td><%=row%></td> 
				<td><a href="ame_bud_est_exp_uprpt.jsp?upto_month=<%=upto_month%>&pyear=<%=inp_year%>&circle_id=<%=circle_id%>&region_id=<%=region_id%>&Office_id=<%=Office_id%>"><%=office_name%></a></td>				
				...
				{ 
					...
					{
						amt_res_up=item_obj.getAmt_net_regprv(Integer.parseInt(Office_id),4,year,prv_month,next_year,3);
					}
					double amt_res_cur=item_obj.getAmt_net_regprv(Integer.parseInt(Office_id),Integer.parseInt(upto_month),next_year,Integer.parseInt(upto_month),next_year,3);
	...
	<tr><th colspan="2"  align="right">Total</th>
	 ...
	 <tr>
			<td colspan="<%out.print(col_count+9);%>" align="center">
			...
			<input type="hidden" name="upto_month" value="<%=upto_month%>"/>
			<input type="hidden" name="inp_year" value="<%=inp_year%>"/>
			<input type="hidden" name="Office_id" value="<%=circle_id%>"/> 
