...
<script type="text/javascript"><!--
			function report(a)
			{
				...
				var upto_month=document.amebudestexprpt.upto_month.value;
				var parms="?flag_action="+flag_action+"&pyear="+pyear+"&Office_id="+Office_id+"&upto_month="+upto_month;
				 if (a==1)					
					document.location.href='ame_bud_est_exp_rpt.jsp'+parms;
				if (a==2)					
					document.location.href='ame_bud_est_exp_reg_uprpt.jsp'+parms;
			}  
...
<table width="100%"
	style="border-collapse: collapse; border-color: skyblue" align="center"
	border=1 cellpadding="5" cellspacing="0">
	<tr>
		<th colspan="3" align="center" class="tdH">Estimate , Budget and
		Item Wise Expenditure <label id='msg'></label></th>
	</tr>
	<tr>
		<td width="25%"><label class="lbl">Office Name :</label></td>
		<td align="left" colspan="2"><label class="lbl"><%=Office_Name%></label></td>
	</tr>

	<tr>
		<td><label class="lbl">Financial Year</label></td>
		<td colspan="3"><select class="select" name="pyear"
			style="width: 100pt;">
	...
	<tr>
		<td><label class="fnt"> Upto Month </label></td>
		<!--	  	          
			  	 onchange="process()" 
			 	 -->
		<td colspan="2" width="63%"><select class="select"
			id="upto_month" name="upto_month" style="width: 90pt;">
	...
	<tr class="tdH">
		<th align="center" colspan="4"><input type="hidden" name="sub"
			value="Report" onclick="report(1)" class="btn"> <input
			type="button" name="sub" value="View" onclick="report(2)" class="btn">
		<input type="button" value="Exit" class="btn" onclick="window.close()" />
		</th>
	</tr>
</table>
<input type="hidden" id="flow_" name="flow_" value="2" /> <input
	type="hidden" name="Office_id" value="<%=Office_id%>" /> <input
	type="hidden" name="flag_action" value="0" /> <input type="hidden"
	name="row_count" value="0" /> <input type="hidden" name="rowcount2"
	value="0" /> <input type="hidden" name="flag_c" value="<%=flag_c%>" />
<%
	} catch (Exception e) {
