...
<script type="text/javascript">
	...
	function reload(office_id,circle_office_id,region_office_id,pyear)
	{
		 window.open("Ame_scheme_Budget_HO.jsp?flag=C&office_id="+office_id+"&circle_office_id="+circle_office_id+"&region_office_id="+region_office_id+"&pyear="+pyear);
...
<% 
		...
		in lakhs) </td>
			...
			String query=" select distinct office_id, office_name from pms_dcb_reg_cir_div_sch where circle_office_id="+obj.setValue("circle_office_id",request)+" order by office_id ";
			ResultSet rs=obj.getRS(query);
			while(rs.next())
			{ 
				String office_id=rs.getString("office_id");
				StringBuffer qry=new StringBuffer();	 
				out.println("<tr><td align='center' width='5%'>"+(++row)+"</td><td align='left'  width='45%'><a href=javascript:reload("+office_id+","+obj.setValue("circle_office_id",request)+","+obj.setValue("region_office_id",request)+",'"+pyear+"')>"+obj.isNull(rs.getString("office_name"),1)+"</td>");
				
				
				qry.append(" select sum(amt.bdamt) as bdamt,amt.region_office_id,amt.circle_office_id,office_id,(select office_name from com_mst_all_offices_view   where office_id=amt.office_id) from "); 
				...
				qry.append(" FROM pms_ame_trn_budget a WHERE fin_year='"+pyear+"' GROUP BY office_id )amt where circle_office_id="+obj.setValue("circle_office_id",request)+" and office_id="+office_id+" group by region_office_id,circle_office_id,office_id");							 							 
