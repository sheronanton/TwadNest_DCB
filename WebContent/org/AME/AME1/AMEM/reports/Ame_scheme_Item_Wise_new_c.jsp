...
<script type="text/javascript">
	...
	function reload(region_office_id,circle_office_id,pmonth,pyear)
	{
		 window.open("Ame_scheme_Item_Wise_new_o.jsp?flag=C&circle_office_id="+circle_office_id+"&region_office_id="+region_office_id+"&pmonth="+pmonth+"&pyear="+pyear);
...
<% 
		...
		<div id='dataid'>
			...
			{								
					out.println("<td align='center' width='5%' valign='top'>"+data1.getDisplayinfo()+"</td>"); 
			}
			out.println("</tr>");
			query=" select distinct circle_office_id as circle_office_id,region_office_id, circle_name from pms_dcb_reg_cir_div_sch where region_office_id="+region_office_id+" order by circle_office_id";
			rs=obj.getRS(query);
			while(rs.next())
			{
				String circle_office_id=rs.getString("circle_office_id"); 
				out.println("<tr><td align='center' width='10%'>"+(++row)+"</td><td align='left'><a href='javascript:reload("+region_office_id+","+circle_office_id+","+pmonth+","+pyear+")'>"+obj.isNull(rs.getString("circle_name"),1)+"</td>");				
				for(Pms_Ame_Masters data1 : list) 
				{				
					...
					String cond=" where GROUP_SNO="+gsno+" AND MAIN_ITEM_SNO="+msno+" and SUB_ITEM_SNO="+ssno+" and month="+pmonth+" and  year="+pyear+" and    circle_office_id="+circle_office_id+"   "  ;				
					String amount="0";				 
					amount=obj_data.getGSValue("PMS_AME_ITEM_WISE_DATA","bill_amt",cond,"circle_office_id");
						out.println("<td align='right' width='5%'>"+df.format(Double.parseDouble(obj.isNull(amount,1)))+"</td>");
				} 
				out.println("</tr>");
			 } 
			 %> 
