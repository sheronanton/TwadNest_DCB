...
<script type="text/javascript">
	...
	{
		 window.open("Ame_scheme_Item_Wise_new_c.jsp?flag=C&region_office_id="+region_office_id+"&pmonth="+pmonth+"&pyear="+pyear);
...
<% 
	...
	//	pmonth="5";
		...
		<div id='dataid'>
			...
			out.println("</tr>");
			query=" select distinct region_office_id as region_office_id,region_name from pms_dcb_reg_cir_div_sch order by region_office_id";
			rs=obj.getRS(query);
			while(rs.next())
			{
				String reg_id=rs.getString("region_office_id");
				out.println("<tr><td align='center'  width='10%'>"+(++row)+"</td><td align='left'><a href='javascript:reload("+reg_id+","+pmonth+","+pyear+")'>"+obj.isNull(rs.getString("region_name"),1)+"</td>");				
				for(Pms_Ame_Masters data1 : list) 
				{				
					int msno=data1.getMainsno();
					int ssno=data1.getSubsno(); 
					int gsno=data1.getGroupsno();
					String cond=" where  GROUP_SNO="+gsno+" and MAIN_ITEM_SNO="+msno+" and SUB_ITEM_SNO="+ssno+"  and month="+pmonth+" and  year="+pyear+" AND region_office_id="+reg_id+"   "  ;				
					String amount="0";				 
						amount=obj_data.getGSValue("PMS_AME_ITEM_WISE_DATA","bill_amt",cond,"region_office_id");
