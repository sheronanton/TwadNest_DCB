...
<%
		...
		if ("5000".equalsIgnoreCase(Office_id))
		...
		String query="SELECT PERFORM_DESC_SNO,PERFORM_DESC ,DISPLAYORDER,TO_BE_PRINTED,UNITS,datatype FROM PMS_AME_MST_DESC " +
		  			" WHERE ACTIVE_STATUS='A'  AND to_be_printed  ='Y' order by  DISPLAYORDER";
		...
		{				
				...
				html+="<td align='center'>"+data1.getPerformdesc()+data1.getUnits()+"</td>"; 
		}
		rs=null;
		int row=0;
		...
		while(rs.next())
		{
			datahtml+="<tr><td align='center'>"+(++row)+"</td><td align='left'>"+obj.isNull(rs.getString("sch_name"),1)+"</td>";
			for(Pms_Ame_Mst_Desc data1 : list) 
			{				
				String performancesno=data1.getPerformdescsno();
				
				String cond=" where MONTH ="+pmonth+" AND YEAR="+pyear+" AND office_id="+Office_id+" and sch_sno="+rs.getString("sch_sno")+" and PERFORM_DESC_SNO="+performancesno;
				
				String amount="0";
				
				if ("1".equalsIgnoreCase(data1.getDatatype()) ||  "2".equalsIgnoreCase(data1.getDatatype()))
				{
					amount=obj_data.getValue("PMS_AME_TRN_SCH_PERFORM_MTH","QTY_OR_AMOUNT",cond);
					datahtml+="<td align='right'>"+df.format(Double.parseDouble(obj.isNull(amount,1)))+"</td>";
				}else
				{
					amount=obj_data.getValue("PMS_AME_TRN_SCH_PERFORM_MTH","REMARKS",cond);
					
					datahtml+="<td align='left'>"+obj.isNull(amount,2)+"</td>";
					amount="0.0";
				}
