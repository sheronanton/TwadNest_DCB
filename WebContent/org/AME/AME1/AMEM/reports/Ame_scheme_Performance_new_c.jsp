...
<script type="text/javascript">
	...
	{
		 window.open("Ame_scheme_Performance_new_o.jsp?flag=C&region_office_id="+region_office_id+"&circle_office_id="+circle_office_id+"&pmonth="+pmonth+"&pyear="+pyear);
	}
</script>
</head>
<body>
...
<% 
		...
		if ("C".equalsIgnoreCase(flag))
		{
			...
			<div id='dataid'>
				...
				{			 					
						out.println("<td align='center' width='7%'>"+data1.getPerformdesc()+data1.getUnits()+"</td>"); 
				}   
				out.println("</tr>");  
				...
				while(rs.next()) 
				{
					String cir_id=rs.getString("circle_office_id");
					String reg_id=rs.getString("region_office_id");
					out.println("<tr><td align='center'>"+(++row)+"</td><td align='left'><a href='javascript:reload("+reg_id+","+cir_id+","+pmonth+","+pyear+")'>"+obj.isNull(rs.getString("circle_name"),1)+"</td>");				
					for(Pms_Ame_Mst_Desc data1 : list) 
					{				
						String performancesno=data1.getPerformdescsno();				
						String cond=" where  PERFORM_DESC_SNO="+performancesno+" AND circle_office_id="+cir_id+" and MONTH ="+pmonth+" AND YEAR="+pyear ;
						String amount="0";				 
						if ("1".equalsIgnoreCase(data1.getDatatype()) ||  "2".equalsIgnoreCase(data1.getDatatype()))
						{  
							amount=obj_data.getValue("pms_ame_cir_sch_sum","QTY_OR_AMOUNT",cond); 
