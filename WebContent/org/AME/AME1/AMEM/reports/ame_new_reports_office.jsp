...
try
{
	...
	String	userid = (String) session.getAttribute("UserId");
		...
		String office_cond="";
			if (Integer.parseInt(region_office_id)==5000)
				...
				office_cond=" office_id in (select office_id from com_mst_all_offices_view where office_level_id='DN' and circle_office_id="+circle_office_id+" and region_office_id="+region_office_id+")  and ";
	...
	String sel="",sely="";
%>   
<form action="./ame_new_reports_office.jsp" method="get">
...
<br>
			...
			try
			{
		 	...
		 	//	table1+=" </table>";
		 		...
		 		if (button_app.equals("View"))
		 		{
				 		...
				 		String sch_sno="0";
				 		String temp_sno="0";
				 		String qry_1=" select office_id ,office_name from com_mst_all_offices_view "+ 
				 			" where office_level_id='DN' and circle_office_id="+circle_office_id+" and  region_office_id="+region_office_id+" and office_id in (select office_id from PMS_DCB_DIV_DIST_MAP ) and "+  
				 			 " office_id in (select office_id from PMS_AME_TRN_ABSTRACT where fin_year='"+fin_year+"' )  ";
						...
						double row_sum=0.0f;
					 	while(rs1.next())
					 	{
					 		...
					 		String sch_cnt=obj2.getValue("PMS_AME_REGION_SCH_COUNT","sum(SCH_COUNT)"," where office_id="+office_id);	
							net_sch_cnt+=Integer.parseInt(sch_cnt); 
					 		table1+="<tr><td valign='top'>"+erow+"</td><td width='10%' valign='top'><a href=ame_new_reports_dv.jsp?fin_year="+fin_year+"&Office_id="+office_id+"&b1=View>"+rs1.getString("office_name")+"</a></td><td align='center'>"+sch_cnt+"</td>";
					 		for(int i=0;i<row;i++)
					 		{
					 			String amt=obj3.getValue("PMS_AME_TRN_ABSTRACT","sum(AM_EST_AMT)"," where  sub_item_sno="+ssnoc[i]+" and main_item_sno="+msnoc[i]+" and office_id="+office_id+" and fin_year='"+fin_year+"' group by office_id ");
			...
			<tr>     
				...
				<input type="button" name="Report" value="Excel Download" onclick="window.open('ame_new_reports_off_excel.jsp?fin_year=<%=fin_year%>&circle_office_id=<%=circle_office_id%>&region_office_id=<%=region_office_id%>','Abstract ','width=400,height=200')">
