...
<%   
	...
	response.setHeader ("Content-Disposition", "attachment;filename=\"Region_Data.xls\"");
	try  
	{
		...
		String table2="<tr><th width='3%' valign='top'>Sl.No</th><th width='10%' valign='top'>Region Name</th><td width='5%' valign='top'>Total <br> Maint.<br> Schemes</td>";
				int row=0;
				int cols=1;
				String prv_msno="0"; 
				String main_desc="";
				String sub_desc="";       
			 	...
			 	while(rs1.next())
			 	{
			 		...
			 		for(int i=0;i<row;i++)
			 		{
			 			String amt=obj3.getValue("PMS_AME_TRN_ABSTRACT ","sum(AM_EST_AMT)"," where  sub_item_sno="+ssnoc[i]+" and main_item_sno="+msnoc[i]+"  AND office_id  in (SELECT office_id  FROM com_mst_all_offices_view WHERE region_office_id="+reg_office_id+") and fin_year='"+fin_year+"' ");
