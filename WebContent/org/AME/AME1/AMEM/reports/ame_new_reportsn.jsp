...
try
{
	...
	int year = cal.get(Calendar.YEAR);
		...
		String office_cond="";
			...
			if (Integer.parseInt(Office_id)==5000)
				...
				office_cond=" Office_id="+Office_id+" and ";
	...
	String sel="",sely="";
%>   
<form action="./ame_new_reportsn.jsp" method="get">
<table align="center" border="1" width="100%">
		...
		<tr>
		 ...
		 <td width="25%"><input type=submit value="View" name="b1" onclick="set_flash()">	<label id='msg'></label></td>
...
<br>
			...
			try
			{
				...
				String sub_desc="";
		 		...
		 		int data_count=obj.getCount("PMS_AME_TRN_ABSTRACT"," where "+office_cond+" fin_year  ='"+fin_year+"'");
		 		if(fin_year.length()>6)
		 		{
			 		if (data_count==0 )
			 		{ 
			 			out.println("<table>"+table1+"  "+table2+"<th>&nbsp;</th></tr><tr><td colspan='"+row+"' align='left'><font color='red' size='3'><strong>Data Not found !!!!!!</strong></font> </td></td></table> ");
			 		}
					 else
					{	
				 	...
				 	//	table1+=" </table>";
				 		...
				 		if (button_app.equals("View"))
				 		{
						 		...
						 		String sch_sno="0";
						 		String temp_sno="0";
						 		String qry_1=" select a.sch_name as sch_name,a.sch_sno as sch_sno from pms_sch_master a where  exists    (SELECT distinct SCHEME_SNO FROM PMS_DCB_MST_BENEFICIARY_METRE where SCHEME_SNO=a.sch_sno and METER_STATUS='L'  and "+office_cond+"  (1=1) )";
								...
								double row_sum=0.0f;
							 	while(rs1.next())
							 	{
							 		...
							 		System.out.println(sch_sno);
							 		table1+="<tr><td valign='top'>"+erow+"</td><td width='10%' valign='top'>"+rs1.getString("sch_name")+"</td>";
							 		CallableStatement smt=con.prepareCall("{call PMS_AME_ITEM_ABSTRACT(?,?,?,?,?,?)}");
							 		for(int i=0;i<row;i++)							 		{
							 			String amt="0";//obj3.getValue("PMS_AME_TRN_ABSTRACT","AM_EST_AMT"," where "+office_cond+"  sub_item_sno="+ssnoc[i]+" and main_item_sno="+msnoc[i]+" and sch_sno="+sch_sno+" and fin_year='"+fin_year+"'  ");
							 									 			
							 			smt.setInt(1,ssnoc[i]);
							 			smt.setInt(2,msnoc[i]);
							 			smt.setInt(3,Integer.parseInt(sch_sno));
							 			smt.setString(4,"'"+fin_year+"");
							 			smt.setString(5,"'"+Office_id+"");
							 			smt.registerOutParameter(6,java.sql.Types.DOUBLE);
							 			try
							 			{
							 			smt.execute();
							 			
							 				amt =Double.toString(smt.getDouble(6));
