...
function transaction(type, processcode) {
	...
	if (sel_type == 1) 
	{
		...
		if (process_code == 5) {
			...
			params = "pyear="+pyear+"&process_code=" + process_code + "&type=" + type
					+ "&sch_sno=" + sch_sno;
			flag = 0;
		}      
		if (process_code == 12) {
	...
	if (sel_type == 2 || sel_type == 4) {
		...
		if (process_code == 1) {
			var rowcount = document.getElementById("rowcount").value;
			params = "type=" +type +"&SCH_SNO="
					+document.getElementById("sch_sno").value +"&pyear="
	...
	if (sel_type == 6) {
		if (process_code == 1) {
			...
			params = "process_code=" +process_code +"&type=" +type
					+"&subitem=" +subitem +"&mainitem=" +mainitem;
	...
	if (sel_type == 2 || sel_type == 4) {
		...
		else if (process_code == 11) 
		{
			...
			if (already==0)
			{
					...
					document.getElementById("famt0").value = roundNumber(
							parseFloat(dqty) * days, 3);
					document.getElementById("famt1").value = roundNumber(
							parseFloat(pqty) * days, 3);
			...
			if (parseFloat(netamt) > 0) {
				...
				document.getElementById("famt" +flag_dmd).value = roundNumber(
						parseFloat(netamt) / 100000, 2);
				// old is 5 and new is
			} else {
	...
	if (sel_type == 3) {
		if (process_code == 5 || process_code == 11 || process_code == 14) 
		{
			...
			for ( var i = 0; i < len; i++) {
				...
				// ","","left","","");
				var SUB_ITEM_DESC_cell = cell("TD", "input", "text",
						"SUB_ITEM_DESC_cellEM_DESC" +(i +1), SUB_ITEM_DESC,
						2, "tb4", "", "", "", "center", "", "");
		...
		else if (process_code == 6) 
		{
			...
			for ( var i = 0; i < len; i++)
			{
				...
				{
					amt_cell = cell("TD", "input", "text", lbl, 0, 6, "", "",readonly, "25%", "left", fun_e, fun_n);
				}
				if (parseInt(DATATYPE) == 4) 
				{
		...
		// Budget Estimate  DELETE  Response
			...
			//	alert("Record Already Exists for Selected Financial Year and Scheme")
				...
				for ( var i = 0; i < len; i++) 
				{
					...
					var TECH_REF_NO = xmlValue(bR, "TECH_REF_NO", i);
