...
<body>
   ...
   <%
    ...
    String sch_name="";
   String ben_typeid="",type_ben="",id="",src_comp="",sub_src_comp="",Office_id="0",page_label="";
	try 
	{
		...
		if(Office_id == null)
	 	...
	 	sch_name="SELECT  "
			 ...
			 +" JOIN"
			 +"( SELECT DISTINCT BENEFICIARY_SNO,"			 
			 +" office_id,scheme_sno"  
			 +"  FROM PMS_DCB_MST_BENEFICIARY_METRE"
			 +" WHERE METER_STATUS='L'"
			 +" AND office_id     = "+Office_id+" and SCHEME_SNO="+id+" )met"
	...
	%>
			...
			<table border="1" width="100%" > <tr>
				<td width="20%"  >
					Name of Habitation /<br>Location
				</td>
				<td   width="10%">
					Capacity of OHT&nbsp;(LL)
				</td>
	...
	String ben_sno="",ben_id="";
			while(rs.next()) 
			{
				...
				out.println("<tr><td>"+i+"</td><td>"+rs.getString("ben_name")+"&nbsp;&nbsp; </td><td><table  border=0 width='100%'><tbody id='c"+row+"'><tr><td width='20%'><input type=text name='loc"+row+"1' id='loc"+row+"1' onkeypress='validateNumber(this)'></td><td  width='10%'><input type=text name='cap"+row+"1' id='cap"+row+"1'  size='7' maxlength='7' onkeypress='validateNumber(this)'></td><td  width='10%'>"
						+"<input type='hidden' id='ben_sno"+row+"' name='ben_sno"+row+"' value="+ben_sno+"><input type='hidden' id='ben_id"+row+"' name='ben_id"+row+"' value="+ben_id+"><input type='hidden' id='rowc"+row+"' name='rowc"+row+"' value=1><input type=text name='qty"+row+"1' id='qty"+row+"1'  size='7'  maxlength='7' onkeypress='validateNumber(this)'></td><td width='20%'><input type=button value='Add Location' id=but"+row+" onclick=addrow("+row+",1)><input type='button' value='SUBMIT' onclick='add("+row+",1)'>"+"&nbsp;"+"<input type=button value='DELETE' id=cid > </td></tr></tbody></table></td></tr>");
					i+=1;
			}
