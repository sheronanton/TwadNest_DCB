var xmlHttp;
function addition_booster() {
	var flag=0;
	var  flag1=val_check('hp','Enter Horse Power');
	var flag2=val_check('duty_type','Enter Duty value'); 
	var flag3=val_check('head','Enter Head value');
	var flag4=val_check('qty','Enter Quantity value');
	var flag5=val_check('reid','Enter Remark');
	...
	{
	url = "../../../../../Booster_station?comment=add&sno=" + sno +  "&pump_type=" + pump_type
	        + "&no_pump="  + no_pump +"&comp=" + comp
			+ "&sub_comp=" + sub_comp + "&hp=" + hp + "&duty_type=" + duty_type
			+ "&head=" + head + "&qty=" + qty + "&remark=" + remark+"&t="+Math.random();
...
function display_booster() {  
	...
	xmlHttp = createObject();
	var url = "";
	url = "../../../../../Booster_station?comment=view&sno=" + sno + "&comp=" + comp + "&sub_comp=" + sub_comp+"&t="+Math.random();
	xmlHttp.open("GET", url, true);
	xmlHttp.onreadystatechange = function() 
	{
...
function view_result(xmlHttp) {
	...
	for ( var i = 0; i < len; i++)
	{
		...
		var PMS_ASSET_BOOSTER_PS_SNO = xmlValue(bR, "PMS_ASSET_BOOSTER_PS_SNO", i);
		...
		var PUMPSET_NO = xmlValue(bR, "PUMPSET_NO", i);
		var type_desc = xmlValue(bR, "TYPE_DESC", i);
		
		
		var sno_cell = cell("TD", "input", "hidden", "PMS_ASSET_BOOSTER_PS_SNO"	+ (i + 1), PMS_ASSET_BOOSTER_PS_SNO, 2, "tdText", "", "", "2%", "", "","");				
		...
		var type_cell = cell("TD", "input", "hidden", "TYPE_OF_PUMPSET" + (i + 1), TYPE_OF_PUMPSET,	2, "", "", "", "", "", "", "");
       ...
       var remark_cell = cell("TD", "label", "", "REMARKS" + (i + 1), REMARKS,	2, "", "", "", "", "", "", "");		
		...
		var pumpno_cell = cell("TD", "label", "", "PUMPSET_NO" + (i + 1), PUMPSET_NO,	2, "", "", "", "", "", "", "");					
		
		new_row.appendChild(sno_cell);
		new_row.appendChild(href_cell);
		new_row.appendChild(pumpno_cell);
...
function delete_booster() {
	var sno = document.getElementById("sid").value;
	var url = "";
	xmlHttp = createObject();
	url = "../../../../../Booster_station?comment=pop&sno=" + sno+"&t="+Math.random();
	xmlHttp.open("GET", url, true);
	xmlHttp.onreadystatechange = function() {
...
function update_booster() {
	...
	if (flag1==0 && flag2==0 && flag3==0 && flag4==0 && flag5==0 )
	{
	url = "../../../../../Booster_station?comment=modify&sno=" + sno +  "&pump_type=" + pump_type
	        + "&no_pump="  + no_pump +"&comp=" + comp
			+ "&sub_comp=" + sub_comp + "&hp=" + hp + "&duty_type=" + duty_type
			+ "&head=" + head + "&qty=" + qty + "&remark=" + remark+"&t="+Math.random();
	xmlHttp.open("GET", url, true);
...
function clear()
{
	 ...
	 document.getElementById('nofboo_main').value="";
