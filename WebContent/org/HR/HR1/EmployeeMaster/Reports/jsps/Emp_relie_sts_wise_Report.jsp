...
int tyr=fyr+1;
 int colspan=3; 
  ...
  t_date=request.getParameter("t_date");
 ...
 // Title=request.getParameter("Title");
  ...
  System.out.println("frm_date--->"+f_date);
  System.out.println("to_date--->"+t_date);
     ...
     <tr>
      <td colspan="2" align="center" valign="top" class="table" width="55%">
       <table border="1" width="100%">
        <tr>
          	<td colspan="3" class="tdH" align="center"><b>Employee Status wise Details Report </b></td>
        </tr>
        <tr class="tdH">
...
"  mst.employee_status_id " +
"FROM " +
"  (SELECT COUNT(*) AS status, " +
"    employee_status_id " +
"  FROM HRM_EMP_CURRENT_POSTING " +
"  WHERE EMPLOYEE_STATUS_ID IN('SAN','DIS','RES','MEV','VRS','DTH','CMR') " +
"  AND DATE_EFFECTIVE_FROM BETWEEN TO_DATE('"+f_date+"','dd-mm-yyyy') AND TO_DATE('"+t_date+"','dd-mm-yyyy') " +
...
"ORDER BY status";
        ...
        tot=tot+total;
        	  ...
        	  if((sl%2)==0)
        	  {
        		  out.print("<tr ><td>"+sl+"</td><td>"+results.getString("EMPLOYEE_STATUS_DESC")+"</td><td><a href='Emp_details_relieval_board_Status_wise_Report.jsp?employee_status_id="+results.getString("employee_status_id")+"&emplo_desc="+results.getString("EMPLOYEE_STATUS_DESC")+"'>"+results.getString("status")+"</a></td></tr>");
        	  }
        	  else
        	  {
        		  out.print("<tr bgcolor='#b6eaff'><td>"+sl+"</td><td>"+results.getString("EMPLOYEE_STATUS_DESC")+"</td><td><a href='Emp_details_relieval_board_Status_wise_Report.jsp?employee_status_id="+results.getString("employee_status_id")+"&emplo_desc="+results.getString("EMPLOYEE_STATUS_DESC")+"'>"+results.getString("status")+"</a></td></tr>");
        	  }
          }
