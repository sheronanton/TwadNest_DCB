 ...
 function getDesignation()
  ...
  
  {
     ...
     /* try{
       ...
       try
       {
       if(document.HRM_EmpSearch.design.length>1)
       {
           for(i=0;i<document.HRM_EmpSearch.design.length;i++)
           {
                   if(document.HRM_EmpSearch.design[i].checked==true)
                	   design= design+document.HRM_EmpSearch.design[i].value +",";               
           }
       ...
       try
       {
    	   if(document.HRM_EmpSearch.include_retire.checked==true)
    		   {
    		   include_retire="checked";
       ...
       //alert("include_retire-->"+include_retire)
       ...
       var cell=document.getElementById("divpre");
        ...
        cell.style.display="none";
        cell.innerText='';
       var url="../../../../../EmpServicePopupServ.view?Command=Emp1&cmbsgroup=" + group+"&include_retire="+include_retire ;
       url=url+"&cmbdes="+design+"&txtEmpName="+name;
       //alert(url);
  ...
  
  {
    ...
    function getEmployeeGpf1()
    {
        ...
        var cell=document.getElementById("divpre");
         ...
         cell.style.display="none";
         cell.innerText='';
        var url="../../../../../EmpServicePopupServ.view?Command=GPF1&txtgpf=" + group+"&include_retire="+include_retire ;
...
function test ()
{
    ...
    var servicegroup="";
        ...
        servicegroup=document.HRM_EmpSearch.cmbsgroup.value;
      
    var url="../../../../../EmpDetailsRep_QueryBased_orderby_combo?command=loadDesignation1&servicegroup="+servicegroup;
    //alert(url)
    var req=getTransport();
    req.open("GET",url,true);
    req.onreadystatechange=function()
...
function elementSelectAll(id)
{
  ...
  if(id=="cmbsgroup")
  {
      ...
      if(document.HRM_EmpSearch.service_group)
      {
    	
            for(i=0;i<document.HRM_EmpSearch.service_group.length;i++)
            {
            	 
                    document.HRM_EmpSearch.service_group[i].checked=true;
    ...
    {
        if(document.HRM_EmpSearch.design)
        {
            if(document.HRM_EmpSearch.design.length>1)
            {
                for(i=0;i<document.HRM_EmpSearch.design.length;i++)
                {
                        document.HRM_EmpSearch.design[i].checked=true;
                }
            }
            else
            {
                document.HRM_EmpSearch.design.checked=true;
            }
        }
    }
}
function UncheckAll(id){ 
	if(document.HRM_EmpSearch.design.length>1)
    {
        for(i=0;i<document.HRM_EmpSearch.design.length;i++)
        {
                document.HRM_EmpSearch.design[i].checked=false;
        }
    }
} 
        ...
        if(v[i].checked==true)
        {
          ...
          //  try{self.opener.doFunction('loademp','null');}catch(e){}
           ...
           /// 
            j=(page-1)*10;
            Minimize();
            var id=items[0]=service[i+j].getElementsByTagName("empid")[0].firstChild.nodeValue;
        	var name=items[1]=service[i+j].getElementsByTagName("empname")[0].firstChild.nodeValue;
        	var intial=items[2]=service[i+j].getElementsByTagName("initial")[0].firstChild.nodeValue;
        	Minimize();
            opener.doParentEmp(v[i].value,name,intial);
            //opener.focus();
            return true;
        }
