...
var s=0;
  ...
  {
   // alert('test');
        var url="../../../../../Emp_Ctrl_Officepopup?Command=Emp";
             ...
             if(service)
                ...
                for(i=p;i<service.length&& c<__pagination;i++)
                {
                     c++;
                        ...
                        items[2]=service[i].getElementsByTagName("designation")[0].firstChild.nodeValue;

                        
                       items[3]=service[i].getElementsByTagName("gpf")[0].firstChild.nodeValue;
                        if(items[3]==0)
                            items[5]="";
...
if(v)
{
    ...
    for(i=0;i<v.length;i++)
    {
    	if(v[i].checked==true)
        {
        	j=(page-1)*10;
            Minimize();
            id= service[i+j].getElementsByTagName("empid")[0].firstChild.nodeValue;
        	name = service[i+j].getElementsByTagName("empname")[0].firstChild.nodeValue;
//            initial = service[i+j].getElementsByTagName("initial")[0].firstChild.nodeValue;
            desig = service[i+j].getElementsByTagName("designation")[0].firstChild.nodeValue;
            opener.doParentEmp(v[i].value,name,desig);
            self.close();
            return true;
        }
    }
