...
function show()
{
 ...
 //	}
      if (document.Hrm_TransJoinForm.typecredit[0].checked==true)
    {
			...
			if(window.opener.document.Hrm_TransJoinForm.missing_type[1].checked)
			{
...
// check condition alert for retired after slipyear
	            		   var response = confirm("Relative month & year is greater than Retired date.. Do u want to Proceed ");
	            		   int_year=document.getElementById("int_year").value;
	            		   if (response == true)
	            		      {
		            		  // int_year=document.getElementById("int_year").value;
...
//      else
 ...
 // alert(rel_month);
    // opener.document.getElementById("fullamount").value=parseInt(opener.document.getElementById("fullamount").value)+parseInt(amount);
     opener.addtable1(headcode,gpfno,spl_ac_head,amount,remarks,rel_year,rel_month,int_year,int_month,slipPrint,Install_no,totalNOInstall);
...
//	      else
	    ...
	    // opener.document.getElementById("fullamount").value=parseInt(opener.document.getElementById("fullamount").value)+parseInt(amount);
	     opener.addtable1(headcode,gpfno,spl_ac_head,amount,remarks,rel_year,rel_month,0,0,slipPrint,Install_no,totalNOInstall);
	      ...
	      if(xmlhttp.readyState==4)
	      {
			...
			var currRow = opener.document.getElementById("tlist").rows.length;
		 	...
		 	if(parseInt(fullamount)>parseInt(opener.document.getElementById("amount").value))
		 	{
		 		...
		 		alert('Amount should not greater than Missing amount');
...
//     else 
    ...
    opener.updatetable1(headcode,gpfno,spl_ac_head,amount,remarks,rel_year,rel_month,seq,int_year,int_month,slipPrint,Install_no,totalNOInstall); 
...
//     else 
    ...
    opener.updatetable1(headcode,gpfno,spl_ac_head,amount,remarks,rel_year,rel_month,seq,0,0,slipPrint,Install_no,totalNOInstall); 
...
function acheadcode(){
	xmlhttp=getxmlhttpObject();
	   ...
	   if(acheadcode!=""){
       url="../../../../../GPF_Missing_DB_CR?command=achead&acheadcode="+acheadcode;
       url=url+"&sid="+Math.random();
       xmlhttp.open("GET",url,true);
      xmlhttp.onreadystatechange=stateChanged;
       xmlhttp.send(null); 
