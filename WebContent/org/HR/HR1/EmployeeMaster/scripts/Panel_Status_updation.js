...
var url="../../../../../Panel_Status_updation?Command=getPanel&cadre="+cadre;
 //alert("Admin   ->"+  url);
...
function Update_status()
{
	...
	if(val == true)
		...
		var panel_id=document.getElementById("panel_reg_no").value;
		var panel_status="";
		if(document.Panel_Status_updation.panel_status[0].checked==true)
			{
			panel_status='Yes';
			}
		else if(document.Panel_Status_updation.panel_status[1].checked==true)
			{
			panel_status='No';
			}
		url="../../../../../Panel_Status_updation?Command=Update_status&cadre_id="
			+ cadre_id +"&panel_id="+panel_id+"&panel_status="+panel_status;
		//alert("url is--->"+url);
		req.open("POST",url,true);              
...
function validation()
{
	...
	if(document.getElementById("panel_reg_no").selectedIndex==0)
	{
	alert("Please Select Panel Id");
	return false;
	}
 return true;
}
