 ...
 function loadvalue(empid)
    {      
         ...
         //var ulength=document.Hrm_TransJoinForm.userid.length;
          ...
          var access_status=rcells.item(5).firstChild.nodeValue;
         // alert(access_status);
          if(access_status=="y"||access_status=="Y")
          {
        	  access_status="Y";  
          }
          else if(access_status=="n"||access_status=="N")
          {
        	  access_status="N";  
          }
       
          if (document.Hrm_TransJoinForm.access_status[0].value==access_status)
          {
              document.Hrm_TransJoinForm.access_status[0].checked=true;
          }
          else
           {
        	  document.Hrm_TransJoinForm.access_status[1].checked=true;
           }   
...
if(xmlhttp==null)
{
    ...
    return;
        ...
        url="../../../../../Revoke_Closed_Office?command=Add&officeid="+officeid+"&unit="+unit+"&userid="+userid+"&date="+date;
    ...
    else if(command=="Update")
    {
                if(nullCheck())
                {
                 ...
                 var access_status="";
                    if (document.Hrm_TransJoinForm.access_status[0].checked)
                    {
                    	access_status=document.Hrm_TransJoinForm.access_status[0].value;
                    }
                    else
                     {
                    	access_status=document.Hrm_TransJoinForm.access_status[1].value;
                     }     
                    
        url="../../../../../Revoke_Closed_Office?command=Update&officeid="+officeid+"&unit="+unit+"&userid="+userid+"&access_status="+access_status+"&date="+date;
        url=url+"&sid="+Math.random();
        xmlhttp.open("GET",url,true);
        xmlhttp.onreadystatechange=stateChanged;
        xmlhttp.send(null);   
    ...
    else if(command=="Delete")
    {
        ...
        url="../../../../../Revoke_Closed_Office?command=Delete&sno="+sno;
        url=url+"&sid="+Math.random();
        xmlhttp.open("GET",url,true);
        xmlhttp.onreadystatechange=stateChanged;
        xmlhttp.send(null);      
    ...
    {   
        ...
        url="../../../../../Revoke_Closed_Office?command=get";
        url=url+"&sid="+Math.random();
        xmlhttp.open("GET",url,true);
       xmlhttp.onreadystatechange=stateChanged;
        xmlhttp.send(null); 
...
function nullCheck()
{
        ...
        else if((document.Hrm_TransJoinForm.userid.value=="--Select User Login Id --"))
        {
