...
<title>DCB Data UnFreeze Request</title>
...
<body>  
<form action="unfreeze_request_new.jsp" name="freezerequest" method="post" onsubmit="return rld()"> 
<%
...
//			Office_id=obj.getValue("HRM_EMP_CURRENT_POSTING", "OFFICE_ID", "where EMPLOYEE_ID in ( select EMPLOYEE_ID	 from SEC_MST_USERS where USER_ID='"+userid+"')") ;
		    ...
		    if (smonth=="0" || syear=="0")
		    {
		    	...
		    	prv_year=obj.prv_year(Integer.parseInt(syear),Integer.parseInt(smonth));
		   		prv_month=obj.prv_month(Integer.parseInt(syear),Integer.parseInt(smonth));
		    } else
		    {
		   		prv_year=Integer.parseInt(syear);
		    	prv_month=Integer.parseInt(smonth);
		   	}
			...
			try  
			{
			    ...
			    freeze_status=obj.getCount("PMS_DCB_DATA_FREEZE", "where cashbook_month="+prv_month+" and cashbook_year="+prv_year+" and  OFFICE_ID="+Office_id);
			    freeze_on=obj.getValue("PMS_DCB_DATA_FREEZE", "to_char(UPDATED_DATE,'dd/mm/yyyy')","frdate","where cashbook_month="+prv_month+" and cashbook_year="+prv_year+" and  OFFICE_ID="+Office_id);
			    ///freeze_on=(freeze_on.equalsIgnoreCase("0")?"":" on " +freeze_on);
			    freeze_by=obj.getValue("PMS_DCB_DATA_FREEZE", "UPDATED_BY_USER_ID","id","where cashbook_month="+prv_month+" and cashbook_year="+prv_year+" and  OFFICE_ID="+Office_id);
			    if (!freeze_by.equalsIgnoreCase("0"))
			    { 
			    	freeze_by=freeze_by.substring(4);  
			    	freeze_by=obj.isNull(obj.getValue("HRM_MST_EMPLOYEES", "EMPLOYEE_NAME","where GPF_NO="+freeze_by),3);
			    }
			    freeze_by=obj.isNull(freeze_by,5);
			    freeze_on=obj.isNull(freeze_on,5);
			    already_freezed=obj.getCount("PMS_DCB_DATA_FREEZE", "where UNFREEZE_REQUEST='Y' and cashbook_month="+prv_month+" and cashbook_year="+prv_year+" and  ACCOUNTING_FOR_OFFICE_ID="+Office_id);
				...
				String sub=obj.setValue("b1",request);
						...
						{ 
							if (already_freezed==0)
							{
								...
								String ins_qry="update PMS_DCB_DATA_FREEZE set UNFREEZE_REQUEST=? , REMARKS=?,REQUEST_USER_ID=?, REQUEST_DATE=clock_timestamp() where cashbook_month="+smonth+" and cashbook_year="+syear+" and  OFFICE_ID="+Office_id;
								...
								{%>
									<script type="text/javascript">
									alert("DCB Data UnFreeze Request Sent to Head Office");
									</script>
						  		<%} 
								r_s=0;
						} else
						  		{
						  			%>
									<script type="text/javascript">
									alert("DCB Data UnFreeze Request Not Sent to Head Office \n \t\t\t\t\tOr \n Request already sent   ");
									</script>
						  		<%
						  		}
				 }
		...
		<table border="1" width="65%" align="center" cellpadding="5" cellspacing="0" style="border-collapse: collapse" >
			<tr  bgcolor="#11297C">
			      <td align="center" colspan="2" style="color: white;">
			       		DCB Data Unfreeze Request <br>   <jsp:expression>Office_Name</jsp:expression> <font class="tdText" color="red"><label id="msg"></label></font>
			     </td>
			</tr>
			<tr class="tdText"> 
		  ...
		  <tr>
		 		<td>Year</td>
		 		<td>		 
         			<select class="select" id="year" name="year"  style="width:100pt"  onchange="report_period_verify_2(document.getElementById('smonth'),this)">
       	...
       	<tr> 
       		<td>Status</td>
       		<% 
       			if (freeze_status==0) out.println("<td><font color=red>DCB Data Not yet Freezed</font></td>");
       			else
       			 out.println("<td><font color=green><b>DCB Data Freezed</b></td>");
       		 %>
       	</tr>
       	<%
       	...
       	<tr>
       		...
       		{
       			out.println("<tr><td>&nbsp;</td><td ><div id='msg_new'><font color='red'>Your Request Already sent to Head office </font></td></tr>");
       			disable="disabled='disabled'";
       		}
       		if (freeze_status==0)
       		{
       			disable="disabled='disabled'";
       			out.println("<tr><td>&nbsp;</td><td ><div id='msg_new'><font color='red'>Request Can't be Rasied </font></td></tr>");
       		}
       		 %>
       		 </div>
       <tr>
       	<td>Reason for Unfreeze<font color=red>**</td>
       	<td>
