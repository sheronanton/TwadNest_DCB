...
<body>  
 <%
 String label="",path="",res="",Lyear="",pyear="",BEN_NAME="",BEN_NO="";
 String user_query="";
 String set1="";  
 String fn="",path_="";
	...
	{  
		...
		obj_new.createStatement(db_con);
		  ...
		  BEN_NO=obj_new.setValue("BEN_NO",request);
		//  BEN_NAME=obj_new.setValue("BEN_NAME",request);
		...
		String BENEFICIARY_NAME=obj_new2.getValue("PMS_DCB_MST_BENEFICIARY","BENEFICIARY_NAME","where BENEFICIARY_SNO="+BEN_NO);
	//	String mvalue=obj_new.month_val(pmonth);
		XmlDataGenerator xml=new XmlDataGenerator(db_con,obj_new);
		user_query="SELECT b.SCH_NAME, " +
				"  a.supplied, " +
				"  a.Demand " +
				"FROM " +
				"  (SELECT SCHEME_SNO, " +
				...
				"    ||'-' " +
				"    ||"+Lyear+",'mm-yyyy') " +
				"  AND BENEFICIARY_SNO="+BEN_NO+" " +
				"  GROUP BY SCHEME_SNO " +
				"  )a " +
				"LEFT OUTER JOIN " +
				"  ( SELECT SCH_SNO,SCH_NAME FROM PMS_SCH_MASTER " +
				"  )b " +
				"ON a.SCHEME_SNO=b.SCH_SNO";
		  fn="";      
...
<table border=1 class="ft" width="100%" cellpadding="6" cellspacing="0" style="border-collapse: collapse;"  align="left"> 
	<tr class="tdH">
		<td colspan=6 align="center" ><%=Office_Name%>Pumped Qty and Demand Statement -<b><%=BENEFICIARY_NAME%></b> 
		<br>
	...
	<tr>  
		...
		<th width='15%' align='center'>Scheme Name</th>
...
<%	
				...
				while(rs_dis2.next())
				{
					String MD_GROUP_DESC=rs_dis2.getString("SCH_NAME");
					if (row==0)
					label="'"+MD_GROUP_DESC+"'";
					else
						label+=",'"+MD_GROUP_DESC+"'";	
				...
				//	String bentype=rs_dis2.getString("bentype");
					String SCH_NAME=rs_dis2.getString("SCH_NAME"); 
				...
				//	tot4+=Double.parseDouble(balance);  
					...
					*/   
					out.println("<td align='left' width='15%' >"+SCH_NAME+"&nbsp;</td>");
