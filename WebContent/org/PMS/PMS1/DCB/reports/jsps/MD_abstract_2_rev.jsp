...
function report(process_code)
{
	...
	if (res!=1)
	{	
	window.open("../../../../../../PMS_DCB_REVIEW_REPORT?schtype="+schtype+"&process_code="+process_code+"&pyear="+pyear+"&pmonth="+pmonth);
	} 
}
</script>
</head>      
...
<%
String  Office_Name="",userid="",Office_id="",path="" ,pmonth="",pyear="",schtype="";
String fn="";  
...
try
{
	...
	pyear=obj.setValue("pyear",request);  
	  schtype=obj.setValue("schtype",request);
	...
	ArrayList ar=obj_abs.getType_wise_new();
	obj_abs.setMonth(Integer.parseInt(pmonth));  
	obj_abs.setYear(Integer.parseInt(pyear));
	...
	if (Integer.parseInt(schtype)==1)
	 ...
	 String res=xml.bar_chart("BEN_TYPE_DESC_SUB",head1,head2);         
...
<tr><td align="center" colspan="6"><input type="button" value="Print" onclick="report(8)"><input type=button value="Back" onclick="window.history.go(-1)">&nbsp;&nbsp;&nbsp;<input type="hidden"  style="color: red;font-weight: bold"  value="Close" onclick="javascript:window.close()"></td></tr>
</table>
