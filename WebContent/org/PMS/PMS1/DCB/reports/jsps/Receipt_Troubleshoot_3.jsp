		...
		<font size=4> Beneficiary Scheme Details</font>
					...
					" FROM "+
						" (SELECT ACCOUNTING_FOR_OFFICE_ID,CASHBOOK_YEAR,CASHBOOK_MONTH,RECEIPT_NO,SUB_LEDGER_TYPE_CODE,SUB_LEDGER_CODE,AMOUNT "+
								  " FROM FAS_RECEIPT_TRANSACTION WHERE ACCOUNTING_FOR_OFFICE_ID="+Office_id+" and  ACCOUNT_HEAD_CODE in (SELECT ACCOUNT_HEAD_CODE  FROM PMS_DCB_RECEIPT_ACCOUNT_MAP where COLLECTION_TYPE in (7,8,9) ) and CASHBOOK_YEAR="+obj.recp_year+" AND CASHBOOK_MONTH="+obj.recp_month+"  "+
						  " and (SUB_LEDGER_TYPE_CODE=0 or SUB_LEDGER_TYPE_CODE is null or SUB_LEDGER_CODE=0 OR SUB_LEDGER_CODE IS NULL )     "+
						  " )trans "+
						  " JOIN "+    
			...
			int row=0;
				...
				while(rs.next())  
				{
					...
					String qry2="select  *  from (SELECT  SL_NO,CASHBOOK_YEAR,RECEIPT_NO,ACCOUNTING_FOR_OFFICE_ID,CASHBOOK_MONTH,SUB_LEDGER_CODE, AMOUNT FROM FAS_RECEIPT_TRANSACTION "+
							" WHERE ACCOUNTING_FOR_OFFICE_ID="+Office_id+" and  ACCOUNT_HEAD_CODE in (SELECT ACCOUNT_HEAD_CODE  FROM PMS_DCB_RECEIPT_ACCOUNT_MAP where COLLECTION_TYPE in (7,8,9) ) and  CASHBOOK_YEAR="+obj.recp_year+" AND CASHBOOK_MONTH="+obj.recp_month+" AND  RECEIPT_NO="+recp_no+
							" AND (SUB_LEDGER_TYPE_CODE=0 or SUB_LEDGER_TYPE_CODE is null or SUB_LEDGER_CODE=0 OR SUB_LEDGER_CODE IS NULL ) ) trans left JOIN ( SELECT SCH_SNO,SCH_NAME, PROJECT_ID FROM PMS_SCH_MASTER"+
					...
					while(rs2.next()) 
					{
						...
						r1++;
						qry_res+="<tr>";
						//qry_res+="<td width='5%'><input type=checkbox id='ch"+r1+"' onchange=load("+project_id+")></td>";
