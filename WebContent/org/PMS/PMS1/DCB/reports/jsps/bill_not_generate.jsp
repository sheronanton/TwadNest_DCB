...
<title>Demand Bill Omission</title>
...
<%
	...
	try {
		...
		qry=" select "+ 
			" mastertab.BENEFICIARY_NAME as BENEFICIARY_NAME,btype.BEN_TYPE_DESC as BEN_TYPE_DESC "+
			" from  "+
			" ( "+
			" SELECT *  FROM pms_dcb_mst_beneficiary  WHERE office_id = "+Office_id+"  AND status = 'L' AND beneficiary_sno not IN "+
			" ( "+
		    "     SELECT beneficiary_sno FROM pms_dcb_monthly_pr  WHERE YEAR="+syear+"  AND MONTH ="+smonth+"  AND office_id =  "+Office_id+
		    " )AND beneficiary_sno not IN (SELECT beneficiary_sno  FROM PMS_DCB_TRN_BILL_DMD  WHERE BILL_YEAR="+syear+"  AND BILL_MONTH = "+smonth+" AND office_id = "+Office_id+") "+
			" ) mastertab "+
			" join  "+
			" ( "+
