...
{	 
	...
	if (fmonth!=0 && fyear!=0 ) 
	{
		if (rtype!=3 && rtype!=6 && rtype!=7 &&  rtype!=1)
		{
				if (b!=0){ 
			    sname=document.getElementById('schtype').options[document.getElementById('schtype').selectedIndex].text
	  			window.open("../../../../../../dcb_statement?oid="+oid+"&process_code="+process_code+"&command="+command+"&fmonth="+fmonth+"&fyear="+fyear+"&bentype="+a+"&schtype="+b+"&sname="+sname+"");
	  			}else
	  			{  
	  				alert("Select Year / Month / Scheme Category ")
	  			} 
	  	}  
	  	else
	  	{ 
...
<%
		...
		try
		{
		  ...
		  combo1=obj.combo_str("PMS_DCB_BEN_TYPE","BEN_TYPE_DESC","BEN_TYPE_ID"," order by BEN_TYPE_ID","bentype","   ");
		...
		String cb=obj.combo_str("PMS_DCB_APPLICABLE_SCH_TYPE","SCH_TYPE_DESC","SCH_TYPE_ID","","rpt","","class=select style='width:250px' ");
				...
				<tr> 
					...
					<td>
						<select id="rtype">  
							...
							<option value="2">Scheme Abstract</option>
							<option value="3">Consolidated Abstract</option> 
							<option value="4">Scheme-Beneficiary Type Abstract</option>
							<option value="5">Scheme-Beneficiary Type</option>
				  ...
				  <tr>
					<td class="tdText" align="left"> <font  class="fnt">Scheme Category</font></td>
					<td><%=combo2%></td>						
				</tr>
				 <tr>
					<td class="tdText" align="left"> <font  class="fnt">Scheme Category</font></td>
