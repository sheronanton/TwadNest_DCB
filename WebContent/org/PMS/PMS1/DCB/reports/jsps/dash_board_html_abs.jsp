...
<%@page import="java.text.DecimalFormat" errorPage="general_error.jsp"%>
...
try
{
	...
	int flag=0;
		...
		{	 
			...
			parameter="&next_up_to_month2="+next_up_to_month2;  
			parameter+="&next_up_to_month1="+next_up_to_month1;   	
			parameter+="&up_to_month="+up_to_month;
			parameter+="&next_year"+next_year;  
			parameter+="&prv_month="+prv_month;
			parameter+="&prv_year="+prv_year;
			parameter+="&year="+year;
			parameter+="&month="+month;	
			parameter+="&current_year_sdate_ap="+current_year_sdate_ap;
			parameter+="&current_year_edate_ap="+current_year_edate_ap;
			parameter+="&current_year_sdate="+current_year_sdate;
			parameter+="&current_year_edate="+current_year_edate;
			parameter+="&sdate1="+week1_sdate;parameter+="&edate1="+week1_edate;
			parameter+="&sdate2="+week2_sdate;parameter+="&edate2="+week2_edate;
			String select_count=obj_new.setValue("select_count", request);
			PreparedStatement  proc_stmt=null;
	 		try
			{
	 				
					proc_stmt = db_con.prepareCall("{call PMS_DCB_DASH_BOARD_PROC (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }");		        		  		 
					proc_stmt.setInt(1, Integer.parseInt(year));
					proc_stmt.setInt(2, Integer.parseInt(month));		        					 
					proc_stmt.setString(3, current_year_sdate_ap);
					proc_stmt.setString(4, current_year_edate_ap);	
					proc_stmt.setString(5, current_year_sdate);
					proc_stmt.setString(6, current_year_edate);
					proc_stmt.setString(7, week1_sdate);
					proc_stmt.setString(8, week1_edate);
					proc_stmt.setString(9, week2_sdate);
					proc_stmt.setString(10, week2_edate);
					proc_stmt.setInt(11,prv_month);
					proc_stmt.setInt(12,april_year);			      
					proc_stmt.setInt(13,next_up_to_month2);
					proc_stmt.setInt(14,next_up_to_month1);
					proc_stmt.setInt(15,next_year);
					proc_stmt.setInt(16,up_to_month);				
					proc_stmt.execute();
		...
		%>
			...
			<tr class="tdH">   
				<td colspan="16" align="center">Statement II showing the status of maintenance of CWSS / Board owned Schemes for the month of  <%=monthval%> <%=year%></td>
			</tr>
			<tr>  
				<th align="center"> Sl.No</th>
				<th> Name of Office</th>
				<th width="5%">Current year <br> Demand from <br>4/<%=april_year%> upto<br><%=prv_month%>/<%=prv_year%></th>  
				<th width="5%">Collection During   4/<%=april_year%></th>
...
<%
			  ...
			  String qry="select  REGION_ID ,sum(CRAMT) as CRAMT,sum(dramt) as dramt,sum(DMDAMT) as DMDAMT,sum(yester_year_demand) as yester_year_demand ,";
			     	 ...
			     	 qry+=" where year="+year+" and month="+month +" group by REGION_ID  order by  REGION_ID";
 			     	int row=0;
		    ...
		    avg3 =((Double.parseDouble(collection_of_current_month)+Double.parseDouble(collection_of_current_month2))/Double.parseDouble(DMDAMT))*100;
		  	pie_chart_xml+="<set name='"+REGNAME+"' value='"+balance+"'/>";
		  ...
		  %>
			  <tr <%=clr%>>
				...
				<td><a href="dash_board_html_reg.jsp?REGION_ID=<%=REGION_ID%><%=parameter%>"><%=REGNAME%></td>
	  ...
	  {%>
		  <tr class="tdH">   
			<td colspan="16" align="center">
						<font color='red' size=4>Data No Available</font>        
			</td>
		</tr>
	<%}
...
pie_chart_xml+="</graph>";  
	...
	<center>
	 ...
	 <div style="width: 100%; height: 37px; text-align: left; vertical-align: top; padding: 0px 0px 0px 7px;" id="chartdiv1" align="center"></div> 
	<%	 
   }catch(Exception r)
