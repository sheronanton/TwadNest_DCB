 ...
 String fn="",path_="";
	...
	try
	{
		...
		XmlDataGenerator xml=new XmlDataGenerator(db_con,obj_new);
		  user_query=" select SCH_TYPE_SUB_DESC  ,SCH_TYPE_ID_SUB, round(  trunc((OPENING_BAL_WC_8+DMD_UPTO_PRV_MNTH_WC_9)-(COLN_UPTO_PRV_MTH_YES_YR_WC_12+COLN_UPTO_PRV_MTH_CR_YR_DMD_13)  ,4),4) as ob, "+
		  " round(trunc(DMD_FOR_MTH_WC_10,4),4) as dmd,round(trunc((COLN_FOR_MTH_YES_YR_WC_14+COLN_FOR_MTH_WC_15),4),4) as collection, "+
		  " round(trunc(BALANCE_18,4),4) as balance   "+
		...
		"  FROM  (  SELECT  DECODE(SUM(OPENING_BAL_WC)  ,NULL,0.0,SUM(OPENING_BAL_WC) ) /100000 AS OPENING_BAL_WC_8, "+
		 ...
		 res=xml.bar_chart("SCH_TYPE_SUB_DESC");           
 %>      
...
<%	
				...
				while(rs_dis2.next())
				{
					...
					out.println("<td align='left' width='45%' ><a href=MD_abstract_2.jsp?pmonth="+pmonth+"&pyear="+pyear+"&schtype="+SCH_TYPE_ID_SUB+">"+SCH_TYPE_SUB_DESC+"&nbsp;</td>");
	 ...
	 out.println(res);
%>  
<tr><td align="center" colspan="6"><input type="button"  style="color: red;font-weight: bold"  value="Close" onclick="javascript:window.close()"></td></tr>
</table> 
