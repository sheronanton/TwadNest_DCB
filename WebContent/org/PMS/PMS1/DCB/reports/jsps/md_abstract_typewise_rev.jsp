...
<center>
 <%
	...
	try
	{
		...
		{
			response.sendRedirect("md_abstract_distwise_rev.jsp?pmonth="+pmonth+"&pyear="+pyear+"&BEN_TYPE_GROUP="+BEN_TYPE_GROUP+"&schtype="+schtype);
		}
		else
		{
		...
		<%
				...
				while(rs_dis.next())  
				{
					  ...
					  user_query="select BENEFICIARY_NAME ,BENEFICIARY_SNO, round(  trunc((OPENING_BAL_WC_8+DMD_UPTO_PRV_MNTH_WC_9)-(COLN_UPTO_PRV_MTH_YES_YR_WC_12+COLN_UPTO_PRV_MTH_CR_YR_DMD_13)  ,3),2) as ob, "+
						...
						" FROM PMS_DCB_LEDGER_ACTUAL  WHERE  YEAR="+pyear+" "+add_cond+" "+off+" AND MONTH ="+pmonth+"  and "+add_cond2+"  and DISTRICT_CODE="+dis_code+" and SCH_TYPE_ID_SUB="+schtype+" GROUP BY YEAR,MONTH, BENEFICIARY_NAME,BENEFICIARY_SNO " +
						" )ac group by YEAR,MONTH,BENEFICIARY_NAME,BENEFICIARY_SNO ) rpt     order by BENEFICIARY_NAME,BENEFICIARY_SNO ";
					 
					  user_query=" SELECT rpt_prv.BENEFICIARY_NAME, ob,dmd,collection,(ob+dmd)-collection AS balance "+ 
					  			" from ( "+ 
							  " SELECT sum(DMD_FOR_MTH_WC)/ 100000 as dmd,((sum(OPENING_BAL_WC)+sum(DMD_UPTO_PRV_MNTH_WC))-(sum(COLN_UPTO_PRV_MTH_YES_YR_WC)+sum(COLN_UPTO_PRV_MTH_CR_YR_DMD))) / 100000 as ob, "+
					          " MONTH,YEAR ,BENEFICIARY_NAME,BENEFICIARY_SNO,DISTRICT_CODE FROM PMS_DCB_LEDGER_ACTUAL  "+
					              " WHERE  MONTH="+prv_month+" AND YEAR="+prv_year+" and  SCH_TYPE_ID_SUB="+schtype+" and "+ add_cond2+" and DISTRICT_CODE="+dis_code+" GROUP BY YEAR,MONTH,BENEFICIARY_SNO,BENEFICIARY_NAME,DISTRICT_CODE  ORDER BY DISTRICT_CODE "+ 
					          " )rpt_prv "+    
					       " join "+
					       " ( "+
					    		   " SELECT  YEAR,MONTH,SUM(COLN_FOR_MTH_YES_YR_WC) + SUM(COLN_FOR_MTH_WC) / 100000 as  collection, BENEFICIARY_NAME,BENEFICIARY_SNO,DISTRICT_CODE from "+ 
					          " PMS_DCB_LEDGER_ACTUAL    WHERE MONTH ="+pmonth+" and year="+pyear+" and "+ add_cond2+"  and DISTRICT_CODE="+dis_code+" and SCH_TYPE_ID_SUB="+schtype+" GROUP BY YEAR,MONTH,BENEFICIARY_SNO,BENEFICIARY_NAME,DISTRICT_CODE "+                                           
					          " ORDER BY DISTRICT_CODE "+  
					          " )rpt_coll "+
