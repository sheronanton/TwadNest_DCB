 ...
 String fn="",path_="";  
	...
	try
	{
		...
		user_query=" SELECT ROUND( TRUNC((OPENING_BAL_WC_8+DMD_UPTO_PRV_MNTH_WC_9)-(COLN_UPTO_PRV_MTH_YES_YR_WC_12+COLN_UPTO_PRV_MTH_CR_YR_DMD_13) ,3),2) AS ob, "+
		  ...
		  " ROUND(TRUNC(BALANCE_18,3),2) AS balance ,rpt.BEN_TYPE_GROUP ,rpt1.BEN_TYPE_GROUP_NAME "+
		  " FROM   ( "+
...
<%	
				...
				while(rs_dis2.next())
				{
					...
					*/
					...
					out.println("<td align='left' width='45%' >"+BEN_TYPE_GROUP_NAME+"&nbsp;</td>");
					out.println("<td align='right' width='10%'>"+df.format(Double.parseDouble(ob.trim()))+"</td>");
					out.println("<td align='right' width='10%'>"+df.format(Double.parseDouble(dmd.trim()))+"</td>");
					out.println("<td align='right' width='10%'>"+df.format(Double.parseDouble(collection.trim()))+"</td>");
					out.println("<td align='right' width='11%'>"+df.format(Double.parseDouble(balance.trim()))+"</td></tr>"); 					  
