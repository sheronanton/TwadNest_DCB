...
<body>  
 ...
 String fn="",path_="";
	...
	{  
		...
		user_query="  select  ROW_NUMBER() Over (Order by TARIFF_RATE) As sl ,TARIFF_RATE,count(BENEFICIARY_SNO) as count from PMS_DCB_WC_SLAB where month="+pmonth+" and year="+pyear+" and Qty>0 group by  TARIFF_RATE";
		  fn="";      
...
<table border=1 class="ft" width="100%" cellpadding="6" cellspacing="0" style="border-collapse: collapse;"  align="left"> 
	<tr class="tdH">
		<td colspan=6 align="center" > <%=Office_Name%>Beneficiary count group by Tariff rate
		<br>
	...
	<tr>  
		...
		<th width='10%' align='center'>Tariff Rate</th>
		<th width='10%' align='center' >Beneficiary count </th>		
...
<%	
			...
			//		if (row==0)
				...
				//	row++;		  
					...
					String ob=rs_dis2.getString("sl");
					String dmd=rs_dis2.getString("TARIFF_RATE"); 
					String collection=rs_dis2.getString("count");
			//		String balance=rs_dis2.getString("balance");
			//		String MD_GROUP=rs_dis2.getString("MD_GROUP");
					 
					if (row1==0)
					{
					set1="["+ Double.parseDouble(ob) +","+Math.round(Double.parseDouble(dmd) )+","+Math.round(Double.parseDouble(collection) )+"]";
					}
					else
					{
						set1+=",["+Math.round(Double.parseDouble(ob) )+","+Math.round(Double.parseDouble(dmd) )+","+Math.round(Double.parseDouble(collection) )+"]";
					}
					row1++;  
				...
				//	out.println("<td align='left' width='45%' ><a href=javascript:ld_1("+MD_GROUP+",'"+MD_GROUP_DESC+"')>"+MD_GROUP_DESC+"&nbsp;</td>");
					out.println("<td align='right' width='10%'>"+df.format(ob)+"&nbsp;</td>");
					out.println("<td align='right' width='10%'>"+df.format(Double.parseDouble(dmd))+"&nbsp;</td>");
					out.println("<td align='right' width='10%'>"+df.format(Double.parseDouble(collection))+"&nbsp;</td></tr>");
