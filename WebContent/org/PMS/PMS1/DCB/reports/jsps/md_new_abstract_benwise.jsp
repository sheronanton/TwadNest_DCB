...
<body>  
 <%
    String  Office_Name="-",userid="0",Office_id="0",pmonth="0",pyear="0" ,BLOCK_SNO="0" ,schtype="0",DISTRICT_CODE="0";
...
<%	
				String user_query=" SELECT ROUND( TRUNC((OPENING_BAL_WC_8+DMD_UPTO_PRV_MNTH_WC_9)-(COLN_UPTO_PRV_MTH_YES_YR_WC_12+COLN_UPTO_PRV_MTH_CR_YR_DMD_13) ,3),3) AS ob,"+
				" ROUND(TRUNC(DMD_FOR_MTH_WC_10,3),3)AS dmd, ROUND(TRUNC((COLN_FOR_MTH_YES_YR_WC_14+COLN_FOR_MTH_WC_15),3),3)AS collection,"+
				" ROUND(TRUNC(BALANCE_18,3),3)AS balance,  BENEFICIARY_NAME "+
				" FROM "+  
...
<tr><td align="center"><input type=button value="Back" onclick="window.history.go(-1)">&nbsp;&nbsp;&nbsp;<a href="md_new_abstract_benwiserpt.jsp?DISTRICT_CODE=<%=DISTRICT_CODE%>&BLOCK_SNO=<%=BLOCK_SNO%>&pmonth=<%=pmonth%>&pyear=<%=pyear%>">Report Excel Download</a></td></tr></table></center>
</body>
</html>