...
function report(process_code)
{
	...
	var BLOCK_SNO=document.getElementById("BLOCK_SNO").value;
	var DISTRICT_CODE=document.getElementById("DISTRICT_CODE").value;
	var res=month_year_check(pmonth,pyear);    
	if (res!=1)  
	{	
	window.open("../../../../../../PMS_DCB_REVIEW_REPORT?process_code="+process_code+"&pyear="+pyear+"&pmonth="+pmonth+"&BLOCK_SNO="+BLOCK_SNO+"&DISTRICT_CODE="+DISTRICT_CODE);
	} 
}
</script>  
</head>
<body>  
 <%
    String  Office_Name="",userid="",Office_id="" ,pmonth="" ,pyear="",BLOCK_SNO="",DISTRICT_CODE="";
	...
	try
	{
		...
		String stype_value=obj_new2.getValue("PMS_DCB_APPLICABLE_SCH_TYPE", "SCH_TYPE_SUB_DESC","where SCH_TYPE_ID_SUB=" + schtype);
		  DISTRICT_CODE=obj_new.setValue("DISTRICT_CODE",request);
		  BLOCK_SNO=obj_new.setValue("BLOCK_SNO",request);		 
		String DISTRICT_CODE_value=obj_new2.getValue("COM_MST_DISTRICTS", "DISTRICT_NAME","where DISTRICT_CODE=" + DISTRICT_CODE);
		String BLOCK_SNO_value=obj_new2.getValue("COM_MST_BLOCKS", "BLOCK_NAME","where BLOCK_SNO=" + BLOCK_SNO);
		...
		if (Integer.parseInt(schtype)==1) 
			...
			add_cond=" ";  
		if (Integer.parseInt(schtype)==1) 
			add_cond2=" and BEN_TYPE_GROUP="+BEN_TYPE_GROUP+"";  
		else
			add_cond2=" and BEN_TYPE_ID_SUB in (6) ";
...
<table border=1  width="72%" cellpadding="5" cellspacing="0" style="border-collapse: collapse;"  align="center"> 
	...
	<tr>
		<td colspan="6" align="left"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>District :</b> 
			<%=DISTRICT_CODE_value%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  	
		&nbsp;&nbsp;&nbsp;<b>Block  :</b> 
			<%=BLOCK_SNO_value%>
		</td>
	...
	<tr> 
		...
		<th width='45%' align='center'>Beneficiary Name</th>
...
<%	
				...
				" ROUND(TRUNC(BALANCE_18,3),2)AS balance,  BENEFICIARY_NAME "+
				" FROM "+  
				...
				"  SUM( DECODE( ac.BALANCE_18 ,NULL,0.0,ac.BALANCE_18))  AS BALANCE_18, ac.MONTH AS MONTH, ac.YEAR AS YEAR ,   BENEFICIARY_NAME  "+
				" FROM "+
				...
				"  SCH_TYPE_ID_SUB,    BENEFICIARY_NAME, MONTH, YEAR FROM PMS_DCB_LEDGER_ACTUAL "+
				"  WHERE YEAR="+pyear+" and BLOCK_SNO="+BLOCK_SNO+"   AND MONTH="+pmonth+" "+add_cond+"   "+add_cond2+"     GROUP BY YEAR, MONTH,SCH_TYPE_ID_SUB, BENEFICIARY_NAME ORDER BY  BENEFICIARY_NAME "+
				"  )ac "+
				" 	GROUP BY YEAR, MONTH, ac.SCH_TYPE_ID_SUB ,  BENEFICIARY_NAME ORDER BY  BENEFICIARY_NAME"+ 
				" )rpt  ORDER BY  BENEFICIARY_NAME";   
				 
user_query=" SELECT BENEFICIARY_SNO,BENEFICIARY_NAME,ob,dmd,collection,(ob+dmd)-collection AS balance "+
" FROM "+ 
	" (SELECT ROUND( TRUNC((OPENING_BAL_WC_8+DMD_UPTO_PRV_MNTH_WC_9)-(COLN_UPTO_PRV_MTH_YES_YR_WC_12+COLN_UPTO_PRV_MTH_CR_YR_DMD_13) ,3),2) AS ob, "+
	    ...
	    " ROUND(TRUNC(BALANCE_18,3),2)   AS balance ,rpt_prv.BENEFICIARY_SNO,rpt_prv.BENEFICIARY_NAME "+
	    " FROM "+
	  " (SELECT DECODE(SUM(OPENING_BAL_WC),NULL,0.0,SUM(OPENING_BAL_WC))/100000 AS OPENING_BAL_WC_8, "+
	    		" DECODE(SUM(DMD_UPTO_PRV_MNTH_WC),NULL,0.0,SUM(DMD_UPTO_PRV_MNTH_WC)) /100000 AS DMD_UPTO_PRV_MNTH_WC_9, "+
	      " DECODE(SUM(DMD_FOR_MTH_WC),NULL,0.0,SUM(DMD_FOR_MTH_WC))      /100000 AS DMD_FOR_MTH_WC_10, "+
	      ...
	      " DECODE(SUM(COLN_UPTO_PRV_MTH_CR_YR_DMD),NULL,0.0,SUM(COLN_UPTO_PRV_MTH_CR_YR_DMD)) /100000 AS COLN_UPTO_PRV_MTH_CR_YR_DMD_13, "+
	      " DECODE(SUM(TOT_COLN_FOR_YR_DMD),NULL,0.0,SUM(TOT_COLN_FOR_YR_DMD))  /100000 AS TOT_COLN_FOR_YR_DMD_17, "+
	      " DECODE(SUM(BALANCE_18),NULL,0.0,SUM(BALANCE_18)) /100000 AS BALANCE_18, BENEFICIARY_SNO,BENEFICIARY_NAME , MONTH,YEAR FROM PMS_DCB_LEDGER_ACTUAL "+
	      " WHERE SCH_TYPE_ID_SUB IS NOT NULL AND BLOCK_SNO="+BLOCK_SNO+" and  YEAR="+prv_year+" AND MONTH="+prv_month+"   AND MD_GROUP=3    AND BEN_TYPE_ID_SUB   IN (6) "+
	    " GROUP BY YEAR, MONTH, BENEFICIARY_SNO, BENEFICIARY_NAME) rpt_prv "+
	    " JOIN "+
	  " (SELECT DECODE(SUM(COLN_FOR_MTH_YES_YR_WC),NULL,0.0,SUM(COLN_FOR_MTH_YES_YR_WC)) /100000 AS COLN_FOR_MTH_YES_YR_WC_14, "+
	    		" DECODE(SUM(COLN_INCLUDE_CHARGES),NULL,0.0,SUM(COLN_INCLUDE_CHARGES))           /100000 AS COLN_FOR_MTH_WC_15, "+
	      " BENEFICIARY_SNO, BENEFICIARY_NAME ,MONTH,YEAR "+
	      " FROM PMS_DCB_LEDGER_ACTUAL "+
	    " WHERE SCH_TYPE_ID_SUB IS NOT NULL "+
	    " AND YEAR="+pyear+" AND MONTH="+pmonth+" AND MD_GROUP=3 AND BLOCK_SNO="+BLOCK_SNO+" AND BEN_TYPE_ID_SUB   IN (6) "+
	    " GROUP BY YEAR,MONTH,BENEFICIARY_SNO,BENEFICIARY_NAME ORDER BY BENEFICIARY_SNO,BLOCK_NAME)rpt_coll "+
	    " ON (rpt_prv.BENEFICIARY_SNO=rpt_coll.BENEFICIARY_SNO) "+  
	  " ORDER BY rpt_prv.BENEFICIARY_SNO, BENEFICIARY_NAME "+
	  " ) ";
				...
				while(rs_dis2.next())
				{
					...
					out.println("<td align='left' width='45%' >"+BENEFICIARY_NAME+"&nbsp;</td>");
...
<tr><td align="left"><input type="button" value="Print" onclick="report(5)"><input type="button" value="Back" onclick="window.history.go(-1)"></td></tr></table></center>
 <input type="hidden" name="pyear" id="pyear" value="<%=pyear%>">
  <input type="hidden" name="pmonth" id="pmonth" value="<%=pmonth%>">
  <input type="hidden" name="BLOCK_SNO" id="BLOCK_SNO" value="<%=BLOCK_SNO%>">
