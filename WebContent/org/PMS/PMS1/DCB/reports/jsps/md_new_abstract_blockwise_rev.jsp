...
function report(process_code)
{
	...
	if (res!=1)  
	{	
	window.open("../../../../../../PMS_DCB_REVIEW_REPORT?process_code="+process_code+"&pyear="+pyear+"&pmonth="+pmonth+"&DISTRICT_CODE="+DISTRICT_CODE);
	} 
}
</script>  
</head>
<body >  
 <%
 	String  Office_Name="",userid="",Office_id="" ,pmonth="",pyear="",DISTRICT_CODE="0" ;
...
<%	
				...
				"    BLOCK_SNO,  BLOCK_NAME, MONTH, YEAR FROM PMS_DCB_LEDGER_ACTUAL "+
				"  WHERE YEAR="+pyear+"  AND MONTH="+pmonth+" "+add_cond+" and DISTRICT_CODE="+DISTRICT_CODE+" and  "+add_cond2+"    GROUP BY YEAR, MONTH, BLOCK_SNO,  BLOCK_NAME ORDER BY  BLOCK_NAME "+
				"  )ac "+
				" 	GROUP BY YEAR, MONTH,  BLOCK_SNO,  BLOCK_NAME ORDER BY  BLOCK_NAME "+
				" )rpt  ORDER BY  BLOCK_NAME";     
			user_query="SELECT BLOCK_SNO, BLOCK_NAME,ob,dmd,collection,(ob+dmd)-collection AS balance FROM "+
			" (SELECT ROUND( TRUNC((OPENING_BAL_WC_8+DMD_UPTO_PRV_MNTH_WC_9)-(COLN_UPTO_PRV_MTH_YES_YR_WC_12+COLN_UPTO_PRV_MTH_CR_YR_DMD_13) ,3),2) AS ob, "+
					" ROUND(TRUNC(DMD_FOR_MTH_WC_10,3),2)  AS dmd, ROUND(TRUNC((COLN_FOR_MTH_YES_YR_WC_14+COLN_FOR_MTH_WC_15),3),2)  AS collection, "+
			  " ROUND(TRUNC(BALANCE_18,3),2) AS balance  ,rpt_prv.BLOCK_SNO,  rpt_prv.BLOCK_NAME "+
			  " FROM "+
			" (SELECT DECODE(SUM(OPENING_BAL_WC),NULL,0.0,SUM(OPENING_BAL_WC))  /100000 AS OPENING_BAL_WC_8, "+
					  " DECODE(SUM(DMD_UPTO_PRV_MNTH_WC),NULL,0.0,SUM(DMD_UPTO_PRV_MNTH_WC))               /100000 AS DMD_UPTO_PRV_MNTH_WC_9, "+
			    " DECODE(SUM(DMD_FOR_MTH_WC),NULL,0.0,SUM(DMD_FOR_MTH_WC))        /100000 AS DMD_FOR_MTH_WC_10, "+
			    ...
			    " DECODE(SUM(COLN_UPTO_PRV_MTH_CR_YR_DMD),NULL,0.0,SUM(COLN_UPTO_PRV_MTH_CR_YR_DMD)) /100000 AS COLN_UPTO_PRV_MTH_CR_YR_DMD_13, "+
			    " DECODE(SUM(TOT_COLN_FOR_YR_DMD),NULL,0.0,SUM(TOT_COLN_FOR_YR_DMD))                 /100000 AS TOT_COLN_FOR_YR_DMD_17, "+
			    " DECODE(SUM(BALANCE_18),NULL,0.0,SUM(BALANCE_18))/100000 AS BALANCE_18,BLOCK_SNO, BLOCK_NAME,MONTH,YEAR "+
			  ...
			  " GROUP BY YEAR,MONTH,BLOCK_SNO, BLOCK_NAME) rpt_prv "+
			  " JOIN "+
			" (SELECT DECODE(SUM(COLN_FOR_MTH_YES_YR_WC),NULL,0.0,SUM(COLN_FOR_MTH_YES_YR_WC)) /100000 AS COLN_FOR_MTH_YES_YR_WC_14, "+
					  " DECODE(SUM(COLN_INCLUDE_CHARGES),NULL,0.0,SUM(COLN_INCLUDE_CHARGES))           /100000 AS COLN_FOR_MTH_WC_15, "+
			    " BLOCK_SNO, BLOCK_NAME ,MONTH, YEAR FROM PMS_DCB_LEDGER_ACTUAL WHERE SCH_TYPE_ID_SUB IS NOT NULL "+
			    " AND YEAR="+pyear+" AND MONTH="+pmonth+" AND MD_GROUP =3  "+add_cond+" and DISTRICT_CODE="+DISTRICT_CODE+" and "+add_cond2+" GROUP BY YEAR,MONTH,BLOCK_SNO, BLOCK_NAME ORDER BY BLOCK_SNO,  BLOCK_NAME )rpt_coll "+
			  " ON (rpt_prv.BLOCK_SNO=rpt_coll.BLOCK_SNO) "+
			 " ORDER BY rpt_prv.BLOCK_SNO,  BLOCK_NAME ) ";
				...
				while(rs_dis2.next())
				{
					...
					out.println("<td align='left' width='45%' ><a href=md_new_abstract_benwise_rev.jsp?DISTRICT_CODE="+DISTRICT_CODE+"&pmonth="+pmonth+"&pyear="+pyear+"&schtype="+schtype+"&BLOCK_SNO="+BLOCK_SNO+">"+BLOCK_NAME+"&nbsp;</td>");
...
<tr><td align="left"><input type="button" value="Print" onclick="report(4)"><input type=button value="Back" onclick="window.history.go(-1)"></td></tr></table></center>
