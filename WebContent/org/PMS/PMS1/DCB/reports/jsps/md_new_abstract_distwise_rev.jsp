...
<table border=1  width="72%" cellpadding="5" cellspacing="0" style="border-collapse: collapse;"  align="center"> 
	...
	<tr>
		...
		<th width='10%' align='center' >Demand<br>   <%=prv_mvalue%>-<%=prv_year%> </th>		
...
<%	
				...
				user_query="SELECT DISTRICT_CODE, DISTRICT_NAME,ob,dmd,collection,(ob+dmd)-collection AS balance FROM "+
				" (SELECT ROUND( TRUNC((OPENING_BAL_WC_8+DMD_UPTO_PRV_MNTH_WC_9)-(COLN_UPTO_PRV_MTH_YES_YR_WC_12+COLN_UPTO_PRV_MTH_CR_YR_DMD_13) ,3),2) AS ob, "+
						" ROUND(TRUNC(DMD_FOR_MTH_WC_10,3),2)  AS dmd, ROUND(TRUNC((COLN_FOR_MTH_YES_YR_WC_14+COLN_FOR_MTH_WC_15),3),2)  AS collection, "+
				  " ROUND(TRUNC(BALANCE_18,3),2) AS balance  ,rpt_prv.DISTRICT_CODE,  rpt_prv.DISTRICT_NAME "+
				  " FROM "+
				" (SELECT DECODE(SUM(OPENING_BAL_WC),NULL,0.0,SUM(OPENING_BAL_WC))  /100000 AS OPENING_BAL_WC_8, "+
						  " DECODE(SUM(DMD_UPTO_PRV_MNTH_WC),NULL,0.0,SUM(DMD_UPTO_PRV_MNTH_WC))               /100000 AS DMD_UPTO_PRV_MNTH_WC_9, "+
				    " DECODE(SUM(DMD_FOR_MTH_WC),NULL,0.0,SUM(DMD_FOR_MTH_WC))        /100000 AS DMD_FOR_MTH_WC_10, "+
				    ...
				    " FROM PMS_DCB_LEDGER_ACTUAL WHERE SCH_TYPE_ID_SUB IS NOT NULL AND YEAR="+prv_year+" AND MONTH="+prv_month+" AND MD_GROUP =3  "+add_cond+" "+off+" and "+add_cond2+""+
				  " GROUP BY YEAR,MONTH,DISTRICT_CODE, DISTRICT_NAME) rpt_prv "+
				  " JOIN "+
				" (SELECT DECODE(SUM(COLN_FOR_MTH_YES_YR_WC),NULL,0.0,SUM(COLN_FOR_MTH_YES_YR_WC)) /100000 AS COLN_FOR_MTH_YES_YR_WC_14, "+
						  " DECODE(SUM(COLN_INCLUDE_CHARGES),NULL,0.0,SUM(COLN_INCLUDE_CHARGES))           /100000 AS COLN_FOR_MTH_WC_15, "+
				    " DISTRICT_CODE, DISTRICT_NAME ,MONTH, YEAR FROM PMS_DCB_LEDGER_ACTUAL WHERE SCH_TYPE_ID_SUB IS NOT NULL "+
				    " AND YEAR="+pyear+" AND MONTH="+pmonth+" AND MD_GROUP =3  "+add_cond+" "+off+" and "+add_cond2+" GROUP BY YEAR,MONTH,DISTRICT_CODE, DISTRICT_NAME ORDER BY DISTRICT_CODE,  DISTRICT_NAME )rpt_coll "+
				  " ON (rpt_prv.DISTRICT_CODE=rpt_coll.DISTRICT_CODE) "+
				 " ORDER BY rpt_prv.DISTRICT_CODE,  DISTRICT_NAME ) ";
				
				
		 
			user_query="select DISTRICT_CODE,district_name,SUM(dmd) AS dmd,SUM(ob) AS ob,SUM(collection) AS collection, "+
			" SUM(OB_MAINT_CHARGES) AS OB_MAINT_CHARGES,SUM(COLN_INT_WC)  AS COLN_INT_WC, "+
			" (SUM(OB_MAINT_CHARGES)+SUM(ob)+SUM(dmd) )-( SUM(collection) +SUM(COLN_INT_WC) ) AS balance "+
			" from  "+
			" ( "+
					" SELECT "+ 
			" ben.BENEFICIARY_TYPE_ID,  btype.BEN_TYPE_DESC, district_name,DISTRICT_CODE,  ben.BENEFICIARY_NAME,  coll.OFFICE_ID ,  dmd,   ob, "+  
			  " collection,OB_MAINT_CHARGES,ob_int,COLN_INT_WC "+
			  " FROM ( "+
					 " (SELECT OFFICE_ID, BENEFICIARY_SNO, "+
					  " (SUM(OB_CUR_YR_WC)+SUM(OB_YESTER_YR_WC))/ 100000 AS ob,sum(OB_MAINT_CHARGES)/ 100000 as OB_MAINT_CHARGES "+ 
			 " FROM PMS_DCB_OB_YEARLY  WHERE  MONTH=5AND FIn_YEAR=2014  GROUP BY OFFICE_ID, BENEFICIARY_SNO "+
			  " )ob "+
			  " LEFT OUTER JOIN "+
			" (SELECT BENEFICIARY_SNO , OFFICE_ID, "+
					  " ((SUM(COLN_CUR_YR_WC) +sum(COLN_MAINT) + SUM(COLN_YESTER_YR_WC) +SUM(ADD_CHARGES_WC)) -SUM(MINUS_CHARGES_WC) )/ 100000 AS collection, "+
			  " sum(CB_INT_AMT_WC) / 100000 as ob_int,sum(COLN_INT_WC) / 100000 as COLN_INT_WC  FROM PMS_DCB_TRN_BILL_DMD   "+
			  " WHERE(to_date((01||'-'||BILL_MONTH||'-'||BILL_YEAR),'dd-MM-yyyy') >=to_date('1-6-2014','dd-mm-yyyy')) "+
			  " AND (to_date((01||'-'||BILL_MONTH||'-'||BILL_YEAR),'dd-MM-yyyy') <=to_date('30-6-2014','dd-mm-yyyy'))  "+
			  " and BENEFICIARY_SNO in (SELECT  BENEFICIARY_SNO FROM PMS_DCB_MST_BENEFICIARY  WHERE status='L'and BENEFICIARY_TYPE_ID_SUB=6) "+
			  " GROUP BY OFFICE_ID, BENEFICIARY_SNO )coll "+
			  " ON coll.OFFICE_ID =ob.OFFICE_ID AND coll.BENEFICIARY_SNO=ob.BENEFICIARY_SNO "+
			  " LEFT OUTER JOIN  (SELECT OFFICE_ID, BENEFICIARY_SNO, SUM(MONTH_BILL_AMT) / 100000 AS dmd  FROM PMS_DCB_TRN_BILL_DMD "+
					" WHERE(to_date((01||'-'||BILL_MONTH||'-'||BILL_YEAR),'dd-MM-yyyy') >=to_date('1-5-2014','dd-mm-yyyy')) "+
			  " AND (to_date((01||'-'||BILL_MONTH||'-'||BILL_YEAR),'dd-MM-yyyy') <=to_date('31-5-2014','dd-mm-yyyy'))  "+
			...
			" LEFT OUTER JOIN "+
			"   (SELECT DISTRICT_CODE , BENEFICIARY_NAME, BENEFICIARY_SNO, BENEFICIARY_TYPE_ID  FROM PMS_DCB_MST_BENEFICIARY   WHERE status='L' and BENEFICIARY_TYPE_ID_SUB=6 "+
					  " )ben "+
			...
			" ) "+
			  " GROUP BY DISTRICT_CODE,  district_name ORDER BY DISTRICT_CODE";	
				...
				{  
					...
					out.println("<td align='left' width='45%' ><a href=md_new_abstract_blockwise_rev.jsp?pmonth="+pmonth+"&pyear="+pyear+"&schtype="+schtype+"&DISTRICT_CODE="+DISTRICT_CODE+">"+DISTRICT_NAME+"&nbsp;</td>");
...
<tr><td align="left"><input type="button" value="Print" onclick="report(3)"><input type=button value="Back" onclick="window.history.go(-1)"></td></tr></table></center>
