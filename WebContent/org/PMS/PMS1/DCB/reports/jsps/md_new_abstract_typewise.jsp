...
<center>
 <%
 	String xml="",xml2=""; 
 	String  Office_Name="",userid="",Office_id=""  ,pmonth="0",pyear="0",BEN_TYPE_GROUP="0",schtype="0";
	...
	try
	{
		...
		{
		 	response.sendRedirect("md_new_abstract_distwise.jsp?pmonth="+pmonth+"&pyear="+pyear+"&BEN_TYPE_GROUP="+BEN_TYPE_GROUP+"&schtype="+schtype);
		}
		else
		{
	...
	<td  width="80%" colspan="2">
		...
		<%		xml="<graph showNames='1' decimalPrecision='0'>";
				xml2="<graph showNames='1' decimalPrecision='0'>";
					...
					while(rs_dis.next())  
					{
						...
						String user_query="select BENEFICIARY_NAME ,BENEFICIARY_SNO, round(  trunc((OPENING_BAL_WC_8+DMD_UPTO_PRV_MNTH_WC_9)-(COLN_UPTO_PRV_MTH_YES_YR_WC_12+COLN_UPTO_PRV_MTH_CR_YR_DMD_13)  ,4),4) as ob, "+
							" round(trunc(DMD_FOR_MTH_WC_10,4),4) as dmd, round(trunc((COLN_FOR_MTH_YES_YR_WC_14+COLN_FOR_MTH_WC_15),4),4) as collection, round(trunc(BALANCE_18,4),4) as balance "+  
		...
		%>  
			...
			<td align="center">
				...
				&nbsp;&nbsp;&nbsp;<a href="md_new_abstract_typewiserpt.jsp?schtype=<%=schtype%>&BEN_TYPE_GROUP=<%=BEN_TYPE_GROUP%>&pmonth=<%=pmonth%>&pyear=<%=pyear%>">Report Excel Download</a>
			</td>
			</tr>
		</table>
		<br> </center></body>
