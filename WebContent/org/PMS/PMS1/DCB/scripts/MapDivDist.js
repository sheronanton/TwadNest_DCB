...
function callServer(command)
 {
       ...
       if(command=="Add")
        {              
    	   			var flag = ( nullCheck('divi') && nullCheck('dis') );
                    if(flag==true)
                    {
	                    url="../../../../../MapDivDist?command=Add&divi="+divi+"&dis="+dis;
        ...
        {
        			var flag = ( (sno!=0) && nullCheck('divi') && nullCheck('dis') );
                    if(flag==true)
                    {
                    	url="../../../../../MapDivDist?command=Update&sno="+sno+"&divi="+divi+"&dis="+dis;
	                    var req=getTransport();
	                    req.open("GET",url,true);        
	                    req.onreadystatechange=function()
	                    {
        ...
        {  
        			...
        			if(flag==true)
        			{
	        			url="../../../../../MapDivDist?command=Delete&sno="+sno;
	        			var req=getTransport();
	                    req.open("GET",url,true);        
	                    req.onreadystatechange=function()
	                    {
        ...
        {           
        			...
        			url="../../../../../MapDivDist?command=Get&divi="+divi+"&dis="+dis+"&page="+page+"&limit="+limit;
		            var req=getTransport();
		            req.open("GET",url,true); 
		            req.onreadystatechange=function()
		            {
        ...
        {               
        			url="../../../../../MapDivDist?command=LoadCmb";
		            var req=getTransport();
		            req.open("GET",url,true); 
		            req.onreadystatechange=function()
		            {
...
//********************************* CallServer Response Coding ***************************************//
                ...
                if(req.status==200)
                {
                    ...
                    else if(cmd == 'LoadCmb')
                    {
                            if(flag=='success')
                            {
                            		...
                            		unloadCombo('dis');
                            		loadCombo('dis','district',baseResponse);
	...
	function edit(rid)
	{
		...
		document.getElementById('dis').value = rcells.item(4).firstChild.nodeValue;
