...
public class ServletValidateAndPublishEvent extends HttpServlet {
        ...
        response.setContentType(CONTENT_TYPE);
            ...
            //String strSecondaryId[]=null;
                  try
                  {
                        ...
                        System.out.println("values retrived sucessfully   :"+str_event_id+"  "+str_event_desc+"  "+str_event_venue+"  "+str_event_date +"  "+str_target_url + "ssss   "+str_event_status);
                  }
                  catch(Exception e)
                  {
                    e.printStackTrace();
                  ...
                  try
                  {
                     ...
                     // update COM_OFFICE_CONTROL set CONTROLLING_OFFICE_ID=?,DATE_EFFECTIVE_FROM=?,REMARKS=?,UPDATED_BY_USER_ID=?,UPDATED_DATE=? where Office_Id=?";  
                      sql1="update com_mst_events set EVENT_DESC=?,EVENT_VENUE=?,EVENT_DATE=?,TARGET_URL=?,EVENT_STATUS_ID=? where EVENT_ID=?";
                     ...
                     ps1.setDate(3,dateOfEvent); 
                      ...
                      ps1.setString(5,str_event_status);
                       System.out.println("id satus"+str_event_status);
                      
                      
                      
                      ps1.setInt(6,event_id);
                     System.out.println("b4 execute");
                     int i=ps1.executeUpdate();            
                     // ps1.close();
                      if(i>=1) {
                          System.out.println("values inserted successfully"); 
                          String msg="Event has been Published Successfully.";
                                                        msg=msg+"<br><br>";
