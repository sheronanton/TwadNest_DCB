...
public class Create_Emp_servlet extends HttpServlet {
  ...
  con.prepareStatement("SELECT EMPLOYEE_ID FROM HRM_MST_EMPLOYEES WHERE EMPLOYEE_ID=?");
                ...
                if (!rs.next()) {
                  ...
                  //  System.out.println("sql control office id"+sql); 
                    if (!rs.next()) {
                        sql="select ACCOUNTING_FOR_OFFICE_ID from HRM_PAY_BILL_GROUP_EMP_LINK where EMPLOYEE_ID=?";
                        ps = con.prepareStatement(sql);
                        ps.setInt(1, eid);
                        rs = ps.executeQuery();
                        System.out.println("sql ACCOUNTING_FOR_OFFICE_ID office id"+sql); 
                        if(rs.next())
                        {
                    ...
                    else
                    {
                    	...
                    	System.out.println("sql for controlling off"+sql);
                    	System.out.println("----->"+OfficeId);
                    }
                        ...
                        if (rs.next()) {
                           ...
                           System.out.println("getting office id is"+offid1);
                            ...
                            if (offid1 != OfficeId) {
                                ...
                                xml = xml + "<flag>failurea</flag><cntrl_Office_name>"+cntrl_Office_name+"</cntrl_Office_name><cntrl_Office_id>"+OfficeId+"</cntrl_Office_id>";
                                flag = false;
                            }
                        } else {
...
con.prepareStatement("select EMPLOYEE_ID,relieval_reason_id from HRM_EMP_RELIEVAL_DETAILS where  EMPLOYEE_ID=?");
                            ...
                            if (reason.equalsIgnoreCase("DTH") || reason.equalsIgnoreCase("TRN") || reason.equalsIgnoreCase("TRT") || reason.equalsIgnoreCase("VRS") || reason.equalsIgnoreCase("LLV") || reason.equalsIgnoreCase("SUS") || reason.equalsIgnoreCase("DPN") || reason.equalsIgnoreCase("STU") || reason.equalsIgnoreCase("DIS") || reason.equalsIgnoreCase("PRO") || reason.equalsIgnoreCase("SAN") || reason.equalsIgnoreCase("STT") || reason.equalsIgnoreCase("RES") || reason.equalsIgnoreCase("ABR") || reason.equalsIgnoreCase("UAL") || reason.equalsIgnoreCase("ABS")) {
