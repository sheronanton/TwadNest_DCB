...
public class Employee_JoinReportValid_Servlet extends HttpServlet {
 ...
 xml + "<flag>failure</flag><flag>Given employee has been working status.Can not post aganin.</flag>";
                    ...
                    /*  else {
                         ...
                         //   try
                         //   {
                            sql="insert into  HRM_EMP_CURRENT_POSTING(EMPLOYEE_ID, OFFICE_ID, DESIGNATION_ID ,DATE_OF_JOINING,REMARKS,DEPARTMENT_ID ) " +
                            "   values( ?,?,?,?,?,'TWAD')";
                                ps=connection.prepareStatement(sql);
                            ...
                            ps.executeUpdate();
                            ps.close();
                            System.out.println("current postin table inserted");
                ...
                if (!flag) {
                    // String sql="Update HRM_EMP_JOIN_REPORTS set FN_OR_AN=?,DESIGNATION_ID=?,POST_COUNTED_ID=?,REMARKS=?,PROCESS_FLOW_STATUS_ID=? where EMPLOYEE_ID=? and JOINING_REPORT_ID=? and OFFICE_ID=? and JOINING_YEAR=?";
                    // ps=connection.prepareStatement(sql);


                    /* find the  SR Controlling Office */
                    int officeid = OffId;
