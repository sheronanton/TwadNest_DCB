...
public class Extension_VRS_Willingness_Details extends HttpServlet {
 ...
 xml + "<dob>" + (rs.getDate("date_of_birth")).getYear() + "</dob>";
    ...
    public void doPost(HttpServletRequest request,
        ...
        int empid=0,offid=0;
        String willingdate="",vrs_reason="",remarks="",Extension_date="",propose_date="",status_date="";
        ...
        try {
        	  ...
        	  if (command.equalsIgnoreCase("Add")) {
        	        ...
        	        Extension_date=request.getParameter("Extension_date");
            ...
            con.commit();
        	  ...
        	  if (command.equalsIgnoreCase("Update")) {
       	       ...
       	       propose_date = request.getParameter("propose_date");
       	        ...
       	        //status_date=request.getParameter("status_date");
        	        System.out.println("status_date is" + Extension_date);
        		   slno = Integer.parseInt(request.getParameter("slno"));    
                  String sql3 ="UPDATE HRM_VRS_WILLINGNESS_MST " +
								"SET remarks=?," +
								"  UPDATED_BY_USER_ID=?, UPDATED_DATE=? ,PROCESS_FLOW_ID=?,EXTENSION_VRS_DATE=to_date(?,'dd/mm/yyyy')" +
								"WHERE REQUEST_SLNO  = ? " +
								"AND EMPLOYEE_ID     = ?";
                  ps = con.prepareStatement(sql3);
                  ps.setString(1, willingdate);
                  ps.setString(2, vrs_reason);
                  //ps.setString(3, Extension_date);
                  ps.setString(1, remarks);
                  ps.setString(2, updatedby);
                  ps.setTimestamp(3, ts);
                  ps.setString(4, "MD");
                  //ps.setString(7, propose_date);
                 // ps.setString(8,status_date);
                  ps.setString(5,Extension_date);
                  ps.setInt(6, slno);
                  ps.setInt(7,empid);
              ...
              String sql3 ="UPDATE HRM_VRS_WILLINGNESS_MST " +
							...
							"  VRS_REQUEST_REASON=?,remarks=?," +
							"  UPDATED_BY_USER_ID=?, UPDATED_DATE=? ,PROCESS_FLOW_ID=?,proposed_vrs_date=to_date(?,'dd/mm/yyyy'),status_updated_date=to_date(?,'dd/mm/yyyy'),EXTENSION_VRS_DATE=to_date(?,'dd/mm/yyyy')" +
							"WHERE REQUEST_SLNO  = ? " +
							"AND EMPLOYEE_ID     = ?";
              ps = con.prepareStatement(sql3);
              ps.setString(1, willingdate);
         	  ...
         	  if (command.equalsIgnoreCase("Load")) {
     	       ...
     	       System.out.println("offid"+offid);
                    String sql3 ="SELECT REQUEST_SLNO, " +
						...
						"    VRS_REQUEST_REASON, " +
						"    VRS_REQUEST_STATUS, " +
						" to_char(EXTENSION_VRS_DATE,'dd/mm/yyyy') as EXTENSION_VRS_DATE, " +
                    ...
                    xml=xml+"<EXTENSION_VRS_DATE>"+rs.getString("EXTENSION_VRS_DATE")+"</EXTENSION_VRS_DATE>" ;
