...
/**
 * Servlet implementation class GPF_Missing_DB_CR
 */
public class GPF_Missing_DB_CR extends HttpServlet {
	private static final long serialVersionUID = 1L;
	 private static final String CONTENT_TYPE = "text/xml; charset=windows-1252";  
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GPF_Missing_DB_CR() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 Connection con=null;
   	  System.out.println("welcome to Missing details");
	        try{
 ...
 if(command.equalsIgnoreCase("add")) {
		            ...
		            /*	System.out.println(gpf_no);   
		            	...
		            	System.out.println(prDate);*/
		            	 ...
		            	 if(slipPrint.equalsIgnoreCase("S"))
				            {
		            	 ps=con.prepareStatement("insert into HRM_GPF_MISSING_CR_DB_MST(ACCOUNTING_UNIT_ID,MISSING_TRANS_TYPE,GPF_NO,AMOUNT,RELATIVE_YEAR,RELATIVE_MONTH,JOURNAL_REF_NO,JOURNAL_REF_DATE," +
		            	 		"UPDATED_BY_USER_ID,UPDATED_DATE,REMARKS,fund_category,PROCESS_FLOW_STATUS_ID,INT_CALCULATED_MONTH,INT_CALCULATED_YEAR,SLIP_PRINT_UNDER) values(?,?,?,?,?,?,?,to_date(?,'dd-mm-yyyy'),?,?,?,?,?,?,?,?)");
			             ps.setInt(1,unit);
		            	 ...
		            	 {
		            		 ps=con.prepareStatement("insert into HRM_GPF_MISSING_CR_DB_MST(ACCOUNTING_UNIT_ID,MISSING_TRANS_TYPE,GPF_NO,AMOUNT,RELATIVE_YEAR,RELATIVE_MONTH,JOURNAL_REF_NO,JOURNAL_REF_DATE," +
		            	 		"UPDATED_BY_USER_ID,UPDATED_DATE,REMARKS,fund_category,PROCESS_FLOW_STATUS_ID,INT_CALCULATED_MONTH,INT_CALCULATED_YEAR,SLIP_PRINT_UNDER,INST_NO,TOT_INST) values(?,?,?,?,?,?,?,to_date(?,'dd-mm-yyyy'),?,?,?,?,?,?,?,?,?,?)");
			             ps.setInt(1,unit);
			             ps.setString(2,missingType);
			             ps.setInt(3,gpf_no);
			             ps.setInt(4,amount);
	          ...
	          //  xml="<response><command>addtrn</command>";
	            ...
	            /*	System.out.println(gpf_no);   
	            	...
	            	ps=con.prepareStatement("select max(HO_JRNL_MST_ID) as jouralid  from HRM_GPF_MISSING_CR_DB_MST ");  
	            	rs=ps.executeQuery(); 
	            	if( rs.next()) 
	            	    		{
	            	...
	            	rs.close();
	            	ps.close();
	            	ps=con.prepareStatement("select fund_category as fund_cate from HRM_GPF_MISSING_CR_DB_MST where HO_JRNL_MST_ID=(select max(HO_JRNL_MST_ID) from HRM_GPF_MISSING_CR_DB_MST) ");  
	            	rs=ps.executeQuery(); 
	            	if( rs.next()) 
	            	    		{
	            	 ...
	            	 ps=con.prepareStatement("select max(HO_JRNL_TRANS_SLNO) as joural_slno from HRM_GPF_MISSING_CR_DB_TRN where HO_JRNL_MST_ID=?");
	            	 ps.setInt(1,jounalId);
		            	...
		            	sl_no=sl_no+1;
		            	System.out.println("sl_no----?"+sl_no);
	            	 ps=con.prepareStatement("insert into HRM_GPF_MISSING_CR_DB_TRN(HO_JRNL_MST_ID,ASSOCIATE_AC_HEADCODE,ASSOCIATE_GPF_NO,ASSOCIATE_TRANS_TYPE,HO_JRNL_TRANS_SLNO,AMOUNT,RELATIVE_MONTH,RELATIVE_YEAR,UPDATED_BY_USER_ID,UPDATED_DATE,PROCESS_FLOW_STATUS_ID,fund_category,INT_CALCULATED_MONTH,INT_CALCULATED_YEAR,SLIP_PRINT_UNDER,INST_NO,TOT_INST) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	            	 ps.setInt(1,jounalId);  
	            	 ps.setInt(2,acHead);
		             ps.setInt(3,gpf_no);
	        ...
	        else if(command.equalsIgnoreCase("checkgpf"))
	        {
	        	...
	        	System.out.print("i am in CheckGPF");
	        	 ...
	        	 boolean flag=false;
	        	 try{
	        		 ps=con.prepareStatement("select * from HRM_GPF_MISSING_CR_DB_MST where GPF_NO=? and MISSING_TRANS_TYPE=? and RELATIVE_YEAR=? and RELATIVE_MONTH=?");  
	        		 ps.setInt(1, gpfNo);
	        		 ...
	        		 if(rs.next()) {
	        		  xml=xml+"<flag>success</flag>";
	        			 ...
	        			 ps=con.prepareStatement("select * from HRM_GPF_MISSING_CR_DB_TRN where ASSOCIATE_GPF_NO=? and ASSOCIATE_TRANS_TYPE=? and RELATIVE_YEAR=? and RELATIVE_MONTH=?");  
		        		 ps.setInt(1, gpfNo);
		        		 ps.setString(2, missType);
		        		 ps.setString(3, relyear);
		        		 ps.setString(4, relmonth);
...
//			        			//check (-/+) OB checks end
	        ...
	        else if(command.equalsIgnoreCase("checkgpf1"))
	        {
	        	...
	        	System.out.print("i am in CheckGPF");
	        	 ...
	        	 xml="<response><command>checkgpf</command>";
	        	 try{
	        		 ps=con.prepareStatement("select * from HRM_GPF_MISSING_CR_DB_MST where GPF_NO=? and MISSING_TRANS_TYPE=? and RELATIVE_YEAR=? and RELATIVE_MONTH=? and HO_JRNL_MST_ID not in ('"+jid+"')");  
	        		 ps.setInt(1, gpfNo);
	        		 ps.setString(2, missType);
	        		 ps.setString(3, relyear);
	        		 ps.setString(4, relmonth);
	        		 ...
	        		 if(rs.next()) {
	        		  xml=xml+"<flag>success</flag>";
	        			 ...
	        			 ps=con.prepareStatement("select * from HRM_GPF_MISSING_CR_DB_TRN where ASSOCIATE_GPF_NO=? and ASSOCIATE_TRANS_TYPE=? and RELATIVE_YEAR=? and RELATIVE_MONTH=? and HO_JRNL_MST_ID not in ('"+jid+"')");  
		        		 ps.setInt(1, gpfNo);
		        		 ps.setString(2, missType);
		        		 ps.setString(3, relyear);
		        		 ps.setString(4, relmonth);
