...
public class Hrm_Emp_Aadhar_updation extends HttpServlet {
 ...
 "<response><command>sessionout</command><flag>sessionout</flag></response>";
        ...
        if(strCommand.equalsIgnoreCase("Update_aadher"))
        {
        	...
        	try
        	{
        		...
        		String ss="UPDATE HRM_MST_EMP_DET_SEC SET AADHAR_NO = ? ,ENCRYPTED_AADHAR =TOOLKIT.encrypt(?) WHERE EMPLOYEE_ID=?";
        		ps=connection.prepareStatement(ss);
        		ps.setString(1, final_aadherno);
        		ps.setString(2, final_aadherno);
        		ps.setInt(3, emp_id);
        		 ps.executeUpdate();
 ...
 "select OFFICE_ID  from HRM_EMP_CURRENT_POSTING where employee_id=?";
        ...
        if(strCommand.equalsIgnoreCase("getaadher"))
        {
        	...
        	{
        		int emp_id = Integer.parseInt(request.getParameter("emp_id"));
        		 sql="SELECT DECODE(AADHAR_NO,NULL,' ',AADHAR_NO) AS AADHAR_NO,DECODE(AADHAR_NO,NULL,' ',TOOLKIT.decrypt(encrypted_aadhar)) AS encrypted_aadhar " +
        				"FROM HRM_MST_EMP_DET_SEC " +
        				"WHERE employee_id=?";
        		 ps=connection.prepareStatement(sql);
        		 ps.setInt(1,emp_id);
        		 result1=ps.executeQuery();
        		 ...
        		 while(result1.next())
        		 {
        			 xml =xml + "<count><AADHAR_NO>" + result1.getString("AADHAR_NO") + "</AADHAR_NO><EAADHAR_NO>" + result1.getString("encrypted_aadhar") + "</EAADHAR_NO></count>";
        		 }
