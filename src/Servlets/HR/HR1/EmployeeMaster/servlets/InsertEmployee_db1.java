...
public class InsertEmployee_db1 extends HttpServlet
{
        ...
        xml="<response><command>ExistgBasic</command>";
                                    ...
                                    if(res.next()) {
                                           ...
                                           if(status.equalsIgnoreCase("CR") || status.equalsIgnoreCase("MD")) {
                                               ...
                                               String ForEmpId="select employee_id, employee_prefix, employee_initial, "+
                                                        " employee_name, gpf_no, EMP_PHOTO_FILE_NAME,photo"+
                                                        " from "+
                                                        " ("+
                                                        ...
                                                        " left outer join"+
                                                        " (" +
                                                        " select employee_id as emp_id, EMP_PHOTO_FILE_NAME,photo  from HRM_EMP_ADDL_PHOTO_NEW_TMP"+
                                                        " where employee_id=?) b "+
                                               ...
                                               int E=0;
                                               while(results1.next()) {
                                                   System.out.println("PAth............"+getServletConfig().getServletContext().getRealPath("web-inf/"+results1.getString("EMP_PHOTO_FILE_NAME")));
                                                   String pa=getServletConfig().getServletContext().getRealPath("WEB-INF/EmpPhotos/"+results1.getString("EMP_PHOTO_FILE_NAME"));
                                               //System.out.println("Photooooooooooo"+results1.getString("EMP_PHOTO_FILE_NAME"));
                                                Blob ablob = results1.getBlob("photo");
                                                   xml=xml+"<flag>EmpDet</flag>";
                                                   xml=xml+"<EmpPref>"+results1.getString("EMPLOYEE_PREFIX")+"</EmpPref><EmpInit>"+results1.getString("Employee_Initial")+"</EmpInit><EmpName>"+results1.getString("Employee_Name")+"</EmpName><EmpGpf>"+results1.getInt("GPF_NO")+"</EmpGpf><EmpPhoto>"+(results1.getString("EMP_PHOTO_FILE_NAME"))+"</EmpPhoto>";
                                                   xml=xml+"<Photo>"+ablob+"</Photo>";
                                               E++;
                                               }
                                               if(E==0) {
                                ...
                                else
                                {
                                       ...
                                       while(results1.next()) {
                                           System.out.println("PAth............"+getServletConfig().getServletContext().getRealPath("web-inf/"+results1.getString("EMP_PHOTO_FILE_NAME")));
                                           String pa=getServletConfig().getServletContext().getRealPath("WEB-INF/EmpPhotos/"+results1.getString("EMP_PHOTO_FILE_NAME"));
                                       System.out.println("Photo"+results1.getString("EMP_PHOTO_FILE_NAME"));
                                           Blob ablob = results1.getBlob("photo");
                                           xml=xml+"<flag>EmpDet</flag>";
                                           xml=xml+"<EmpPref>"+results1.getString("EMPLOYEE_PREFIX")+"</EmpPref><EmpInit>"+results1.getString("Employee_Initial")+"</EmpInit><EmpName>"+results1.getString("Employee_Name")+"</EmpName><EmpGpf>"+results1.getInt("GPF_NO")+"</EmpGpf><EmpPhoto>"+(results1.getString("EMP_PHOTO_FILE_NAME"))+"</EmpPhoto>";
                                           xml=xml+"<Photo>"+ablob+"</Photo>";
