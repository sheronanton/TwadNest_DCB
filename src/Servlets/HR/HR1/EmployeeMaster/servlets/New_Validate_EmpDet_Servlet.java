...
public class New_Validate_EmpDet_Servlet extends HttpServlet {
        ...
        if (StrCommand.equalsIgnoreCase("toValidate")) {
            ...
            try {
               ...
               String strDob1 = dateFormat1.format(d1);
                ...
                System.out.println("date3:" + strDob);
            ...
            try {
                ...
                dateFormat1.applyPattern("yyyy-MM-dd");
                System.out.println("strDob:" + d1);
              String  strRetire1 = dateFormat1.format(d1);
                System.out.println("date2:" + strRetire);
                date11 = Date.valueOf(strRetire1);
                System.out.println("date3:" + strRetire);
            ...
            try {
                ...
                if (dateOfRegPro != null) {
                    ...
                    System.out.println("New Date:" + dateOfRegPro);
                }
 ...
 try {
            ...
            try {
                ...
                if (strDistrict != 999) {
                    try {
                        ...
                        ps.close();
                        System.out.println("First query over");
                        String sqltemp =
                            "update HRM_MST_EMP_ADDL_TMP set Date_Of_Birth=to_date(?,'dd-mm-yyyy'),Gender=?,COMMUNITY_ID=?,Native_District_Code=?,Native_Taluk_Code=?,QUALIFICATIONS=?,EMPLOYMENT_STATUS_ID=?,TWAD_ENTRY_DATE=?,JOIN_TIME_DESIG_ID=?,EMP_CURRENT_STATUS_ID=?,Marital_Status=?,Remarks=?,Other_State=?,Other_Districts=?,PROCESS_FLOW_STATUS_ID=?,PROBATION_DATE=to_date(?,'dd-mm-yyyy'),APPROVED_PROBATIONER=?,UPDATED_BY_USER_ID=?,UPDATED_DATE=?,Employee_Prefix=?,IS_CONSOLIDATE=?,EMP_WKG_GETTING_PENSION=?,IS_HANDICAPPED=?,REGULARISATION_DATE=to_date(?,'dd-mm-yyyy'),RETIRE_DATE=to_date(?,'dd-mm-yyyy')     where EMPLOYEE_ID=?";
                        ...
                        //ps.setInt(16,strEmpId);
                        ps.executeUpdate();
                        System.out.println("second query over");
                        ps.close();
                        String query="select employee_id from HRM_EMP_RETIREMENT_DATA where employee_id=?";
                      ...
                      if(f==1)
                      {  query="update HRM_EMP_RETIREMENT_DATA set DATE_OF_BIRTH=to_date(?,'dd-mm-yyyy'),RETIREDATE=to_date(?,'dd-mm-yyyy'),PROCESS_STATUS_ID=?,UPDATED_DATE=?,UPDATED_BY=?,REMARKS=?  where employee_id=?";
                             
                      ps1=connection.prepareStatement(query);
                      ps1.setString(1, strDob);
                      ps1.setString(2, strRetire);
                      ps1.setString(3, "FR");
                      ...
                      {  query="INSERT " +
                        ...
                        ps1=connection.prepareStatement(query);
                        ps1.setInt(1, strEmpId);
                        ps1.setString(2, strDob);
                        ps1.setString(3, strRetire);
                    ...
                    try {
                        ...
                        String sql =
                            "update hrm_mst_employees set Date_Of_Birth=to_date(?,'dd-mm-yyyy'),Gender=?,COMMUNITY_ID=?,Native_District_Code=?,Other_Districts=?,Other_State=?,QUALIFICATIONS=?,EMPLOYMENT_STATUS_ID=?,TWAD_ENTRY_DATE=?,JOIN_TIME_DESIG_ID=?,EMP_CURRENT_STATUS_ID=?,Marital_Status=?,Remarks=?,Native_Taluk_Code=?,PROCESS_FLOW_STATUS_ID=?,PROBATION_DATE=to_date(?,'dd-mm-yyyy'),APPROVED_PROBATIONER=?,UPDATED_BY_USER_ID=?,UPDATED_DATE=?,Employee_Prefix=?,IS_CONSOLIDATE=?,EMP_WKG_GETTING_PENSION=?,IS_HANDICAPPED=?,REGULARISATION_DATE=to_date(?,'dd-mm-yyyy'),RETIRE_DATE=to_date(?,'dd-mm-yyyy')  where EMPLOYEE_ID=?";
                        ...
                        ps1.setString(23, Handicapped);
                        ps1.setString(22, getting);
                        ps1.setString(24, reg_date);
                        ps1.setString(25, strRetire);
                     ...
                     //   xml = xml + "<flag>success</flag>>";
                        ...
                        System.out.println("Updation of employees details when other district is selected");
                        String sqlttemp =
                            "update HRM_MST_EMP_ADDL_TMP set Date_Of_Birth=to_date(?,'dd-mm-yyyy'),Gender=?,COMMUNITY_ID=?,Native_District_Code=?,Other_Districts=?,Other_State=?,QUALIFICATIONS=?,EMPLOYMENT_STATUS_ID=?,TWAD_ENTRY_DATE=?,JOIN_TIME_DESIG_ID=?,EMP_CURRENT_STATUS_ID=?,Marital_Status=?,Remarks=?,Native_Taluk_Code=?,PROCESS_FLOW_STATUS_ID=?,PROBATION_DATE=to_date(?,'dd-mm-yyyy'),APPROVED_PROBATIONER=?,UPDATED_BY_USER_ID=?,UPDATED_DATE=?,Employee_Prefix=?,IS_CONSOLIDATE=?,EMP_WKG_GETTING_PENSION=?,IS_HANDICAPPED=?,REGULARISATION_DATE=to_date(?,'dd-mm-yyyy'),RETIRE_DATE=to_date(?,'dd-mm-yyyy')  where EMPLOYEE_ID=?";
                        ...
                        ps1.executeUpdate();
                        ps1.close();
                        System.out.println("First Query completed");
                String query="select count(employee_id) from HRM_EMP_RETIREMENT_DATA where employee_id=?";
                ps1=connection.prepareStatement(query);
                ps1.setInt(1, strEmpId);
                results1= ps1.executeQuery();
                        if(results1.next())
                ...
                if (g == 0) {
                    ...
                    if (SessionOfficeId == OfficeId) {
                        ...
                        String sql3 =
                            ...
                            "  a.EMPLOYEE_PREFIX,a.Date_Of_Birth,a.RETIRE_DATE,a.Gender,a.COMMUNITY_ID, " +
  ...
  results4.getDate("Date_Of_Birth").toString().split("-");
                                ...
                                if (results4.getDate("RETIRE_DATE") ==
                                null) {
                                	strRetire = "0";
                            } else {
                                String[] sd;
                                sd =
results4.getDate("Date_Of_Birth").toString().split("-");
                                strRetire = sd[2] + "/" + sd[1] + "/" + sd[0];

                                System.out.println("Date is: " + strRetire);
                            }
 ...
 xml=xml+"<strRetire>"+strRetire+"</strRetire>";                                   
                                    xml =
