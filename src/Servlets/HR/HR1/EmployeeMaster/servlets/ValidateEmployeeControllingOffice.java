...
public class ValidateEmployeeControllingOffice extends HttpServlet {
 ...
 "<response><command>sessionout</command><flag>sessionout</flag></response>";
        ...
        if (strCommand.equalsIgnoreCase("check")) {
            ...
            String sql =
                "insert into HRM_EMP_CONTROLLING_OFFICE_TMP(EMPLOYEE_ID,CONTROLLING_OFFICE_ID,CONTROLLING_DEPARTMENT_ID,DATE_OF_JOINING) values(?,?,?,?)";
 ...
 xml + "<DEPARTMENT_ID>" + rs.getString("CONTROLLING_DEPARTMENT_ID") +
   ...
   "</recordstatus>";
            ...
            try {
                ...
                ps.setInt(7, strEName);
                ps.executeUpdate();


                sql =
 "select * from HRM_EMP_CONTROLLING_OFFICE where EMPLOYEE_ID=?";
                ps = connection.prepareStatement(sql);
                ps.setInt(1, strEmpName);
                ResultSet rs = ps.executeQuery();
                ...
                if (rs.next()) {
                    sql =
 "Update HRM_EMP_CONTROLLING_OFFICE set CONTROLLING_OFFICE_ID=?,CONTROLLING_DEPARTMENT_ID=?,process_flow_status_id=?,CONTROLLING_DATE_FROM=?,UPDATED_BY_USER_ID=?,UPDATED_DATE=? where EMPLOYEE_ID=?";
                    ...
                    sql =
 "insert into HRM_EMP_CONTROLLING_OFFICE(EMPLOYEE_ID,CONTROLLING_OFFICE_ID,CONTROLLING_DEPARTMENT_ID,CONTROLLING_DATE_FROM,PROCESS_FLOW_STATUS_ID,UPDATED_BY_USER_ID,UPDATED_DATE) values(?,?,?,?,?,?,?)";
                    ps = connection.prepareStatement(sql);
                    ps.setInt(1, strEName);
                    ps.setInt(2, strOffName);
                    ps.setString(3, strDeptId);
