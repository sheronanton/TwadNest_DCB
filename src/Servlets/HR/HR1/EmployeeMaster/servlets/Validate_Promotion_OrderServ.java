...
public class Validate_Promotion_OrderServ extends HttpServlet {
   ...
   rs.getString("OFFICE_NAME") + "</oname>";
            ...
            try {
                ...
                while (rs.next()) {
                 ...
                 if(rs.getDate("PROCEEDING_DATE")!=null)
                 {
                 procdate=new SimpleDateFormat("dd/MM/yyyy").format(rs.getDate("PROCEEDING_DATE"));
                 System.out.println("max from date :"+procdate);
                 }
                 else {
 ...
 DriverManager.getConnection(ConnectionString, strdbusername.trim(),
   ...
   new GregorianCalendar(Integer.parseInt(sd[2]), Integer.parseInt(sd[1]) - 1,
        ...
        if (command.equalsIgnoreCase("Validate")) {
            ...
            try {
                String sql =
                    "update HRM_PROMOTION_DETAILS set PROCESS_FLOW_STATUS_ID=?,UPDATED_BY_USER_ID=?,UPDATED_DATE=? where PROMOTION_ISSUE_OFFICE_ID=? and PROMOTION_ORDER_ID=?";
                ...
                if (a > 0) {
                    String sql1 =
                        "update HRM_PROMOTION_ORDERS set PROCEEDING_DATE=?,PROCESS_FLOW_STATUS_ID=?,UPDATED_BY_USER_ID=?,UPDATED_DATE=? where PROMOTION_ISSUE_OFFICE_ID=? and PROMOTION_ORDER_ID=?";
