...
public class EmpTStatus_Servlet extends HttpServlet 
{
    ...
    public void doGet(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException
    {
        ...
        try
        {
          ...
          strEmpDesc = request.getParameter("EmpDesc");
        ...
        if(strCommand.equalsIgnoreCase("Delete"))
                {
                  ...
                  String sql="delete from HRM_MST_EMPLOYMENT_STATUS where EMPLOYMENT_STATUS_ID=?";
                  try
                  {
                ...
                else if(strCommand.equalsIgnoreCase("Update"))
                {
                  ...
                  String sql="update HRM_MST_EMPLOYMENT_STATUS set EMPLOYMENT_STATUS=? where EMPLOYMENT_STATUS_ID=?";
                  try
                  {
                    ps=connection.prepareStatement(sql);
                    ps.setString(2,strEmpId);
                    ps.setString(1,strEmpDesc);
                    ps.executeUpdate();        
                    xml=xml+"<flag>success</flag>";
                    ps.close();
                  }
        ...
        else if(strCommand.equalsIgnoreCase("Add"))
        {
          ...
          String sql="insert into HRM_MST_EMPLOYMENT_STATUS(EMPLOYMENT_STATUS_ID,EMPLOYMENT_STATUS) values(?,?)";
          try
          {
                          
            ps=connection.prepareStatement(sql);
            ps.setString(1,strEmpId);
            ps.setString(2,strEmpDesc);
                            ps.executeUpdate();  
                            xml=xml+"<flag>success</flag>";
                            ps.close();
                        }
          ...
          xml=xml+"</response>";
             ...
             String  sxml="<response><command>check</command>";
               try
              {
                String sql="SELECT * FROM HRM_MST_EMPLOYMENT_STATUS WHERE EMPLOYMENT_STATUS_ID=? AND EMPLOYMENT_STATUS=?";
                try
               {
                   ps=connection.prepareStatement(sql);
                   ps.setString(1,strEmpId);
