...
public class ServletResponseDetails_New extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
        ...
        try 
        {
            ...
            if(res.next()) 
            {
                ...
                java.sql.Date DateOfFormation1 = res.getDate("LAST_UPDATED_DATE");  
                String DateToBeDisplayed1="";
                if(DateOfFormation==null)
                {
                    DateToBeDisplayed1="null";
                }
                else
                {
                    try
                    {
                        java.text.SimpleDateFormat sdf=new java.text.SimpleDateFormat("dd/MM/yyyy");
                        DateToBeDisplayed1=sdf.format(DateOfFormation1);
                    }
                    catch(Exception e)
                    {
                        System.out.println("error while formatting date : " + e);
              ...
              System.out.println("inner"); 
                ...
                xml="<response><flag>success</flag><reportdate>"+DateToBeDisplayed+"</reportdate><lastdate>"+DateToBeDisplayed1+"</lastdate><issuepriority>"+res.getString("issue_priority")+"</issuepriority><majorsystem>"+res.getString("major_system_id")+"</majorsystem><majorsystemdesc>"+res.getString("major_system_desc")+"</majorsystemdesc><minorsystemid>"+res.getString("minor_system_id")+"</minorsystemid><minorsystem>"+res.getString("minor_system_desc")+"</minorsystem><issuetitle>"+res.getString("issue_title")+"</issuetitle><issuedesc>"+res.getString("issue_desc")+"</issuedesc><issuestatus>"+res.getString("issue_status")+"</issuestatus><userid>"+res.getString("REPORTED_BY_USER_ID")+"</userid><solution>"+res.getString("issue_solution")+"</solution>";
                ...
                try{
                       ...
                       ps=connection.prepareStatement("select employee_name||case when employee_initial is null then ' ' else '.'||employee_initial end as employee_name from hrm_mst_employees where employee_id=?");
                       ps.setInt(1,Emp_Id);
                       res=ps.executeQuery();
                       if(res.next())
                       {
