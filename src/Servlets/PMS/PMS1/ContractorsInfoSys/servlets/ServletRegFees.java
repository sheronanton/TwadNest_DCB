...
public class ServletRegFees extends HttpServlet
{
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
  {
   ...
   String sql="insert into PMS_MST_CON_REGN_FEES(Office_Level_Id,Regn_Class_Id,Entire_State,Regn_Fees,Date_effective_from,Last_Updated_Date,Last_Updated_By)values(?,?,?,?,?,?,?)";
      try
       ...
       //first date field for Date_creation field in the form
        ...
        try {
         ...
         ps.setInt(2,cid1);
          ...
          System.out.println("psc is:"+cid1);
          ps.setString(3,strState);
           System.out.println("pss is:"+strState);
          ps.setInt(4,strRegFees);
          System.out.println("psstrr is:"+strRegFees);
          ps.setDate(5,date1);

          System.out.println("psDate is:"+DateTo);
   ...
   //int fees=Integer.parseInt(request.getParameter("regfees"));
      ...
      try
      {
        ...
        String sql="select * from PMS_MST_CON_REGN_FEES where Office_Level_Id=? and Regn_Class_Id=? and Entire_State=?";
        ...
        if(rs.next())
        {
            ...
            xml=xml+"<flag>success</flag>";
            xml=xml+"<value> ";
            xml=xml+"<officeid>"+rs.getString("Office_Level_Id")+"</officeid><classid>"+rs.getInt("Regn_Class_Id")+"</classid><state>"+rs.getString("Entire_State")+"</state>";
            xml=xml+"</value>";
        }
        else
        {
           ...
           String strState=request.getParameter("state");
            ...
            int cid3=0;
             cid3= Integer.parseInt(strRegClassId1);
                ...
                ps=connection.prepareStatement("select * from PMS_MST_CON_REGN_FEES where Office_Level_Id=? and Regn_Class_Id=? and Entire_State=?");
                ps.setString(1,strOffice);
                 ps.setInt(2,cid3);
                ps.setString(3,strState);
                results=ps.executeQuery();
               ...
               Date strdate=results.getDate("Date_effective_from");
                java.text.SimpleDateFormat sdf=new java.text.SimpleDateFormat("dd/MM/yyyy");
                 ...
                 xml=xml+"<flag>success</flag><officeid>"+ results.getString("Office_Level_Id")+"</officeid><classid>" + results.getInt("Regn_Class_Id") + "</classid><state>" + results.getString("Entire_State")  +"</state><fees>" + results.getInt("Regn_Fees")  + "</fees><date>" + StrDate1 + "</date>";
                }
                else
                {
                   xml=xml+ "<flag>failure</flag>";
    ...
    else if(strCommand.equalsIgnoreCase("Update"))
    {
      ...
      catch(Exception e)
      {
          ...
          {
          System.out.println("inside try catch");
          String sql="update PMS_MST_CON_REGN_FEES set Regn_Fees=?,Date_effective_from=?,Last_Updated_Date=?,Last_Upated_By=? where Office_Level_Id=? and Regn_Class_Id=? and Entire_State=?";
            //ps=connection.prepareStatement(sql);
    ...
    else if(strCommand.equalsIgnoreCase("Delete"))
    {
       ...
       System.out.println("inside delete");
        ...
        try{
          ...
          try
          {
              String sql="delete from PMS_MST_CON_REGN_FEES where Office_Level_Id=? and Regn_Class_Id=? and Entire_State=?";
              ps=connection.prepareStatement(sql);
              ps.setString(1,strOffice);
              ps.setInt(2,cid5);
              ps.setString(3,strState);
