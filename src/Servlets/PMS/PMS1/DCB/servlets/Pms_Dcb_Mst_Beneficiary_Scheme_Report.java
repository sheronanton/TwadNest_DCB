...
/**
 * Servlet implementation class Pms_Dcb_Mst_Beneficiary_Scheme_Report
 */
public class Pms_Dcb_Mst_Beneficiary_Scheme_Report extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	String new_cond=Controller.new_cond; 
    public Pms_Dcb_Mst_Beneficiary_Scheme_Report() {
        super();
	...
	{
		int empid=0;
		int oid=0,BeneficiaryType=0,subDivisionId=0,BeneficiaryName=0;
		...
		System.out.println("DCB->Pms_Dcb_Mst_Beneficiary_Scheme_Report->command->"+command+"->BeneficiaryType"+BeneficiaryType+"SubDivision: "+subDivisionId);
        ...
        {
			con=Obj.con();    
            ps_oid = con.prepareStatement("select CASE WHEN OLD_OFFICE_ID IS NULL and DATE_ALLOWED_UPTO IS NULL THEN OFFICE_ID WHEN DATE_ALLOWED_UPTO >= clock_timestamp() and SWITCH_ID is not null THEN SWITCH_ID ELSE OFFICE_ID END AS OFFICE_ID from PMS_DCB_COM_OFFICE_SWITCH where EMPLOYEE_ID =? ");
	...
	public String getReport(Connection con,PreparedStatement ps,Controller Obj,int officeId,int BeneficiaryType,int subDivisionId,int BeneficiaryName)
	{
		...
		try
		{
				ps = con.prepareStatement("select a.BENEFICIARY_SNO,b.beneficiary_name,c.sch_name,a.sub_div_id,d.sch_type_shortdesc,c.sch_sno,count(a.METRE_LOCATION) as count,a.tariff_flag,t.ben_type_desc" +
	        		...
	        		" join PMS_DCB_MST_BENEFICIARY b on b.status='L' and   a.BENEFICIARY_SNO=b.BENEFICIARY_SNO" +
	        ...
	        while(res.next())
	        {
	        	xmlResponse +=generateXML("BENEFICIARY_SNO", res.getString("BENEFICIARY_SNO"),2,Obj);
	        	...
	        	xmlResponse +=generateXML("SchemeSno", res.getString("sch_sno"),1 ,Obj);
