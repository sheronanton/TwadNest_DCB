...
public class Receipt_SL_DCB extends HttpServlet
{
    /**
	 *  
	 */
   ...
   {      
        ...
        else if(strCommand.equalsIgnoreCase("loadEmployee")) 
        {
         ...
         else if(strCommand.equalsIgnoreCase("Load_SL_Code") || strCommand.equalsIgnoreCase("Load_MasterSL_Code") ) 
         {
             ...
             {
                 try {
                	     int other_dept_off_alias_id=0;
                         try{other_dept_off_alias_id=Integer.parseInt(request.getParameter("other_dept_off_alias_id"));}catch(Exception e){System.out.println("getting master combo slcode failed");}
                           ps=con.prepareStatement("select SCH_SNO, SCH_NAME  from PMS_SCH_MASTER where  OFFICE_ID=?  and SCH_SNO in (select SCHEME_SNO from PMS_DCB_MST_BENEFICIARY_METRE where meter_status='L' and  BENEFICIARY_SNO="+other_dept_off_alias_id+") order by SCH_NAME");
                         ps.setInt(1,cmbOffice_code);
                         rs=ps.executeQuery();
                        
                         while(rs.next())
                         {
                         xml=xml+"<cid>"+rs.getInt("SCH_SNO")+
                         "</cid><cname>"+rs.getString("SCH_NAME")+
                         "</cname>";
                         y++;
                         }
                         if(y!=0)
             ...
             {
            	 try {
            		 String BENEFICIARY_TYPE_ID=request.getParameter("bentype");
                     ps=con.prepareStatement("                                     " +
                     "select                                                     \n" + 
                     "    BENEFICIARY_SNO as sl_code,                             \n" + 
                     "    BENEFICIARY_NAME  as sl_code_desc"+//|| (select ' (' || DESCRIPTION ||')' from pms_mst_lookup where lookup_id = BENEFICIARY_TYPE_ID ) as sl_code_desc   \n" + 
                     " from                                                       \n" + 
                     "   PMS_DCB_MST_BENEFICIARY                                     \n" + 
                     "where" +new_cond+
                     "   and  BENEFICIARY_TYPE_ID ="+BENEFICIARY_TYPE_ID+"    and                                                 \n" + 
                     "   DISTRICT_CODE in (                                      \n" + 
         ...
         else if(strCommand.equalsIgnoreCase("sch_code")){
