...
public class RM_WATER_SOURCE_TYPE extends HttpServlet 
{
                      ...
                      HttpServletResponse response) throws ServletException, IOException 
    {
        ...
        { 
            ...
            String sql="";
            String xml="<response>";
            /*sql="insert into RWS_MST_WATER_SOURCE_TYPE values(?,?)";
            try
            {
            ...
            try 
            {
                cs=conn.prepareCall("{call RM_WATER_SOURCE_TYPE(?,?,?,?)}") ; 
        ...
        else if(action.equalsIgnoreCase("dup"))
        {
                ...
                String sql="";                
                String xml="<response>";
                sql="select WATER_SOURCE_TYPE_ID from RWS_MST_WATER_SOURCE_TYPE where WATER_SOURCE_TYPE_ID=?";
        ...
        {
                System.out.println("DUPLICATION 2");
                svtname=request.getParameter("sid");
                String sql="";                
                String xml="<response>";
                sql="select WATER_SOURCE_TYPE from RWS_MST_WATER_SOURCE_TYPE where WATER_SOURCE_TYPE=?";
                ...
                try
                {
                    ...
                    if(rs.next())
                    {  
                       System.out.println("Duplication 2 occur");
                       xml=xml+"<flag>success</flag>";}
                    else
                     {
                       System.out.println("no dup 2 there");
                       xml=xml+"<flag>failure</flag>";
                     } 
                }
                catch(Exception e) 
                {
                    System.out.println("duplication 2 exc:"+e); 
                    xml=xml+"<flag>failure</flag>";
                }
            xml=xml+"</response>";
            System.out.println("xml is:"+xml);
        ...
        else if(action.equalsIgnoreCase("deleting"))
        {
                ...
                String sql="";
                String xml="<response>";
                sql="delete from RWS_MST_WATER_SOURCE_TYPE where WATER_SOURCE_TYPE_ID=?";
                try
                {
        ...
        else if(action.equalsIgnoreCase("updating"))
        {
                ...
                String sql="";
                String xml="<response>";
                //sql="update RWS_MST_WATER_SOURCE_TYPE set WATER_SOURCE_TYPE=? where WATER_SOURCE_TYPE_ID=?";
                /* try
                {
