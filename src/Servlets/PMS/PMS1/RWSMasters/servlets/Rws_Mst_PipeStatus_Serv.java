...
public class Rws_Mst_PipeStatus_Serv extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, 
        ...
        try {
            ...
            strname=request.getParameter("txtDSPipeType");
            strcode=Integer.parseInt(request.getParameter("txtDSPipeTypeid"));
        ...
        if(strCommand.equalsIgnoreCase("Add")) {
            ...
            try
            {
             ...
             CallableStatement cs=con.prepareCall("{call RWS_MST_PIPESTATUS_GENID (?,?,?)}" );
            //cs.setInt(1,strcode);
            ...
            try {
                ps=con.prepareStatement("select * from RWS_MST_DS_PIPE_STATUS where DS_PIPE_TYPE_ID=?");
                ps.setInt(1,strcode);
                rs=ps.executeQuery();
                int i=0;
                while(rs.next()) {
            ...
            try {
                ps=con.prepareStatement("select * from RWS_MST_DS_PIPE_STATUS where DS_PIPE_STATUS=?");
            ...
            try {
                ps=con.prepareStatement("delete from RWS_MST_DS_PIPE_STATUS where DS_PIPE_TYPE_ID=?");
                ps.setInt(1,strcode);
                ps.executeUpdate();
            ...
            try {
                ...
                ps=con.prepareStatement("update RWS_MST_DS_PIPE_STATUS set DS_PIPE_STATUS=? where DS_PIPE_TYPE_ID=?");
                ps.setString(1,strname);
                ps.setInt(2,strcode);
                ps.executeUpdate();
