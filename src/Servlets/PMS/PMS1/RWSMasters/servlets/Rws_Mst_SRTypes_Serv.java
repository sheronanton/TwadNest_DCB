...
public class Rws_Mst_SRTypes_Serv extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, 
       ...
       // String xml="";
        ...
        try {
            ...
            //strcode=Integer.parseInt(request.getParameter("txtSRTypeid"));
            System.out.println("assign....."+strcode);
            strname=request.getParameter("txtSRType");
        ...
        if(strCommand.equalsIgnoreCase("Add")) {
            ...
            /*try {
                ps=con.prepareStatement("insert into RWS_MST_SR_TYPES values(?,?)");
             ...
             try 
                 {
                     cs=con.prepareCall("{call RM_SR_TYPES_PROC(?,?,?,?)}") ; 
                     cs.setInt(1,strcode);
            ...
            strcode=Integer.parseInt(request.getParameter("txtSRTypeid"));
            try {
                ps=con.prepareStatement("delete from RWS_MST_SR_TYPES where SERVICE_RESERVOIR_TYPE_ID=?");
                ps.setInt(1,strcode);
                ps.executeUpdate();
            ...
            /*try {
                ...
                ps=con.prepareStatement("update RWS_MST_SR_TYPES set SERVICE_RESERVOIR_TYPE=? where SERVICE_RESERVOIR_TYPE_ID=?");
...
private void validate_2ndfield() 
    {
               try
               {
                   ps=con.prepareStatement("select * from RWS_MST_SR_TYPES where SERVICE_RESERVOIR_TYPE=?");
                   ps.setString(1,strname);
                   rs=ps.executeQuery();
                   if(!rs.next())
                   {
