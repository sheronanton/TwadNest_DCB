...
public class Rws_Mst_SchPrgsServ extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, 
       ...
       // String xml="";
        ...
        try {
            ...
            strcode=request.getParameter("txtPrgId");
            System.out.println("assign....."+strcode);
            strname=request.getParameter("txtPrgName");
         ...
         if(strCommand.equalsIgnoreCase("Delete")) {
            ...
            sql="DELETE FROM RWS_MST_SCHEME_PROGRAMMES WHERE PROGRAMME_ID=?";
            }
           ...
           /* try {
                ps=con.prepareStatement("INSERT INTO RWS_MST_SCHEME_PROGRAMMES VALUES(?,?)");
            ...
            try 
                {
                    cs=con.prepareCall("{call RM_SCHEME_PROGRAMMES_proc(?,?,?,?)}") ; 
            ...
            try {
                ps=con.prepareStatement("SELECT * FROM RWS_MST_SCHEME_PROGRAMMES WHERE PROGRAMME_ID=?");
                ps.setString(1,strcode);
                rs=ps.executeQuery();
                int i=0;
                while(rs.next()) {
           ...
           /* try {
                ...
                ps=con.prepareStatement("UPDATE RWS_MST_SCHEME_PROGRAMMES SET PROGRAMME_DESC=? WHERE PROGRAMME_ID=?");
 ...
 private void validate_2ndfield() 
    {System.out.println("yes");
        try
        {
            ps=con.prepareStatement("select * from RWS_MST_SCHEME_PROGRAMMES where PROGRAMME_DESC=?");
            ps.setString(1,strname);
            rs=ps.executeQuery();
            if(!rs.next())
            {
