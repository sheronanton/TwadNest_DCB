...
public class Rws_Mst_SchemeStatus_Serv extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, 
        ...
        try {
            ...
            strname=request.getParameter("txtSchemeStatus");
            System.out.println("assign Name....::"+strname);
            strcode=Integer.parseInt(request.getParameter("txtSchemeStatusid"));
        ...
        if(strCommand.equalsIgnoreCase("Add")) {
            ...
            try
            {
             ...
             CallableStatement cs=con.prepareCall("{call RWS_MST_SCHEMESTATUS_GENID (?,?,?)}" );
            //cs.setInt(1,strcode);
          ...
          /*  try {
            ...
            try {
                ps=con.prepareStatement("select * from RWS_MST_SCHEME_STATUS where SCHEME_STATUS_ID=?");
                ps.setInt(1,strcode);
                rs=ps.executeQuery();
                int i=0;
                while(rs.next()) {
            ...
            try {
               System.out.println("novali name:::"+strname);
                ps=con.prepareStatement("select * from RWS_MST_SCHEME_STATUS where SCHEME_STATUS_DESC=?");
            ...
            try {
                ps=con.prepareStatement("delete from RWS_MST_SCHEME_STATUS where SCHEME_STATUS_ID=?");
                ps.setInt(1,strcode);
                ps.executeUpdate();
            ...
            try {
                ...
                ps=con.prepareStatement("update RWS_MST_SCHEME_STATUS set SCHEME_STATUS_DESC=? where SCHEME_STATUS_ID=?");
                ps.setString(1,strname);
                ps.setInt(2,strcode);
                ps.executeUpdate();
                xml=xml+"<flag>success</flag>";
