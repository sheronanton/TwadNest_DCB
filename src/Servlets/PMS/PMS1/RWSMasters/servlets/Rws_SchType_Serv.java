...
public class Rws_SchType_Serv extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, 
       ...
       // String xml="";
        ...
        try {
            ...
            strcode=request.getParameter("txtSchemeId");
            System.out.println("assign....."+strcode);
            strname=request.getParameter("txtSchemeName");
         ...
         if(strCommand.equalsIgnoreCase("Delete")) {
            ...
            sql="DELETE FROM RWS_MST_SCHEME_TYPES WHERE SCHEME_TYPE_ID=?";
            }
            ...
            /*try {
                ps=con.prepareStatement("INSERT INTO RWS_MST_SCHEME_TYPES VALUES(?,?)");
             ...
             try 
                 {
                     cs=con.prepareCall("{call RM_SCHEME_TYPES_proc(?,?,?,?)}") ; 
            ...
            try {
                ps=con.prepareStatement("SELECT * FROM RWS_MST_SCHEME_TYPES WHERE SCHEME_TYPE_ID=?");
                ps.setString(1,strcode);
                rs=ps.executeQuery();
                int i=0;
                while(rs.next()) {
            ...
            /*try {
                ...
                ps=con.prepareStatement("UPDATE RWS_MST_SCHEME_TYPES SET SCHEME_TYPE_NAME=? WHERE SCHEME_TYPE_ID=?");
...
private void validate_2ndfield() 
{
           try
           {
               ps=con.prepareStatement("select * from RWS_MST_SCHEME_TYPES where SCHEME_TYPE_NAME=?");
               ps.setString(1,strname);
               rs=ps.executeQuery();
               if(!rs.next())
               {
