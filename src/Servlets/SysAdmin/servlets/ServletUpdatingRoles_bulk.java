...
public class ServletUpdatingRoles_bulk extends HttpServlet 
{
  ...
  //      System.out.println("servlet called");
        try 
        {
            ...
            ps=connection.prepareStatement("select a.Employee_Id,a.Employee_Name||case when a.employee_initial is null then ' ' else '.'||a.employee_initial end Employee_Name from HRM_MST_EMPLOYEES a " + 
     ...
     //   System.out.println("servlet Login");
       ...
       //     System.out.println("inside login");
        ...
        //    System.out.println("Empid id"+LoginId);
            ...
            ps=connection.prepareStatement("select a.list_seq_no,a.role_id,c.Employee_Name||case when a.employee_initial is null then ' ' else '.'||a.employee_initial end Employee_Name,b.role_name from " + 
            ...
            "b.role_id=a.role_id " + 
            "where c.employee_id=? order by b.role_name,a.role_id");
            ps.setString(1,LoginId);
        ...
        //    System.out.println("Hai");
            ...
            //select the role id from EmployeeId without Assigning him
             ps=connection.prepareStatement("select Role_Id,Role_Name from SEC_MST_ROLES where Role_Id not in(select Role_Id from SEC_MST_USER_ROLES where Employee_Id=?)order by role_name");
             ps.setString(1,LoginId);
             results=ps.executeQuery();
             while(results.next()) {
    ...
    else if(strCommand.equals("Sample"))
    {
    	xml="<response><command>Add</command>";
    	...
    	String emplyee_id=request.getParameter("txtUserId");
    	 String updatedby=(String)session.getAttribute("UserId");
    	roleid=request.getParameterValues("roleid");
    	seq=request.getParameterValues("seq");
        long k=System.currentTimeMillis();
    	Timestamp ts=new Timestamp(k);
    	...
    	"  )";
    	ps=connection.prepareStatement(sql);
    		ps.setInt(1, Integer.parseInt(emplyee_id));
    		ps.setInt(2, Integer.parseInt(roleid[i])); 
    		ps.setString(3,REMARKS);
    		ps.setString(4,updatedby);
            ps.setTimestamp(5,ts);
            ps.setInt(6, Integer.parseInt(seq[i]));
    ...
    else if(strCommand.equals("getName"))
    {
    	
    	
    	//xml="";
    		 xml="<response><command>getName</command>";
    		String emp_id=request.getParameter("emp_id");
    	
    		String sql=" select EMPLOYEE_ID,EMPLOYEE_NAME,DESIGNATION FROM VIEW_EMPLOYEE2 WHERE EMPLOYEE_ID=?";
    		try
    		{
    			ps=connection.prepareStatement(sql);
          ...
          //        System.out.println("Profile:"+profile);
               if(profile==1)
               {
                           ...
                           if(results.next())
                           {
                                      ...
                                      ps=connection.prepareStatement("select Employee_Name||case when employee_initial is null then '' else '.'||employee_initial end Employee_Name from hrm_mst_employees where employee_id=?");
                                      ps.setInt(1,empid);
                                      results=ps.executeQuery();
                                      if(results.next()) {
                                      ...
                                      "b.role_id=a.role_id " + 
                                      "where a.employee_id=? order by b.role_name,a.role_id");
                                      ps.setInt(1,empid);
                                      results=ps.executeQuery(); 
                                      while(results.next()) {
                                  ...
                                  //select the role id from EmployeeId without Assigning him   
                                  ps=connection.prepareStatement("select Role_Id,Role_Name from SEC_MST_ROLES where MAJOR_SYSTEM_ID='"+major+"' and Role_Id not in(select Role_Id from SEC_MST_USER_ROLES where Employee_Id=?) order by role_name");
                                  ps.setInt(1,empid);
                                  results=ps.executeQuery();
                                  while(results.next()) {
                                      xml=xml+"<RoleValue>"+results.getString("Role_Id")+"</RoleValue><RoleName1>"+results.getString("Role_Name")+"</RoleName1>";
                    ...
                    //         System.out.println("test1");
                              ps=connection.prepareStatement("select USER_NAME||case when user_initial is null then ' ' else '.'||user_initial end Employee_Name Employee_Name from SEC_MST_OTHER_USERS_PROFILE where USER_ID=?");
                              ps.setString(1,LoginId);
                              results=ps.executeQuery();
                              if(results.next()) {
                                  xml=xml+"<EmpName>"+results.getString("Employee_Name") + "</EmpName>";
                      //            System.out.println(results.getString("Employee_Name"));
                              ...
                              "b.role_id=a.role_id " + 
                              "where a.USER_ID=? order by b.role_name,a.role_id");
                              ps.setString(1,LoginId);
                              results=ps.executeQuery(); 
                              while(results.next()) {
                          ...
                          //select the role id from EmployeeId without Assigning him   
                          ps=connection.prepareStatement("select Role_Id,Role_Name from SEC_MST_ROLES where MAJOR_SYSTEM_ID='"+major+"' and Role_Id not in(select Role_Id from SEC_MST_OTHER_USER_ROLES where USER_ID=?)order by Role_Name ");
                            ps.setString(1,LoginId);
                          results=ps.executeQuery();
                          while(results.next()) {
                              xml=xml+"<RoleValue>"+results.getString("Role_Id")+"</RoleValue><RoleName1>"+results.getString("Role_Name")+"</RoleName1>";
